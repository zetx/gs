...
#line 47
texture2D Ganossa_MF_NormTex { Width =  1280/2; Height =  720/2; Format = RGBA8; };
texture2D Ganossa_MF_PrevTex { Width =  1280/2; Height =  720/2; Format = RGBA8; };
texture2D Ganossa_MF_QuadFullTex { Width =  1280/2; Height =  720/2; Format = RGBA8; };
texture2D Ganossa_MF_QuadFullPrevTex { Width =  1280/2; Height =  720/2; Format = RGBA8; };
texture2D Ganossa_MF_QuadTex { Width =  1280/2; Height =  720/2; Format = RGBA16F; };
#line 53
...
#line 97
if (!(texcoord.x <= (1.0 / 1280)*2 && texcoord.y <= (1.0 / 720)*2))
discard;
#line 100
float2 coord = float2(0.0,0.0);
#line 102
for (float i = 2.0f; i <  1280/2; i=i+ 1280/192f)
{
coord.x = (1.0 / 1280)*i*2;
#line 106
[unroll]
for (float j = 2.0f; j <  720/2; j=j+ 720/108f )
{
coord.y = (1.0 / 720)*j*2;
float3 quadFull = tex2D(Ganossa_MF_QuadFullColor, coord).xyz;
float quadFullPow = quadFull.x+quadFull.y+quadFull.z;
#line 113
if(i <   1280/2/2 && j <   720/2/2)
quadR.x += quadFullPow;
else if(i >   1280/2/2 && j <   720/2/2)
quadR.y += quadFullPow;
else if(i <   1280/2/2 && j >   720/2/2)
quadR.z += quadFullPow;
else
...
{
float4 Ganossa_MF_Quad = saturate(tex2D(Ganossa_MF_QuadColor, float2((1.0 / 1280), (1.0 / 720)))-0.1f);
#line 130
if (mfDebug)
{
#line 133
