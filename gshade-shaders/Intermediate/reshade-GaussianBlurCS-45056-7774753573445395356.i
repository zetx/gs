...
texture PlaceHolder {Width = 1799; Height = 998; Format = RGB10A2;};
texture Convolution {Width = 1799; Height = 998; Format = RGB10A2;};
#line 57
...
sampleCoord.x += i;
sampleCoord = clamp(sampleCoord, 0, float2(1799, 998));
...
int2 sampleCoord = sampleOffset + groupCoord;
sampleCoord.x += i;
sampleCoord = clamp(sampleCoord, 0, float2(998, 1799));
...
DispatchSizeY =  (int(998 +  2 - 1) / int( 2));
}
pass
{
ComputeShader = VerticalPassCS< 32,  2>;
DispatchSizeX =  (int(998 +  32 *  4 - 1) / int( 32 *  4));
