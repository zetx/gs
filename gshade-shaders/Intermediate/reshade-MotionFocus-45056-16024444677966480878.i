...
#line 47
texture2D Ganossa_MF_NormTex { Width =  792/2; Height =  710/2; Format = RGBA8; };
texture2D Ganossa_MF_PrevTex { Width =  792/2; Height =  710/2; Format = RGBA8; };
texture2D Ganossa_MF_QuadFullTex { Width =  792/2; Height =  710/2; Format = RGBA8; };
texture2D Ganossa_MF_QuadFullPrevTex { Width =  792/2; Height =  710/2; Format = RGBA8; };
texture2D Ganossa_MF_QuadTex { Width =  792/2; Height =  710/2; Format = RGBA16F; };
#line 53
...
#line 97
if (!(texcoord.x <= (1.0 / 792)*2 && texcoord.y <= (1.0 / 710)*2))
discard;
#line 100
float2 coord = float2(0.0,0.0);
#line 102
for (float i = 2.0f; i <  792/2; i=i+ 792/192f)
{
coord.x = (1.0 / 792)*i*2;
#line 106
[unroll]
for (float j = 2.0f; j <  710/2; j=j+ 710/108f )
{
coord.y = (1.0 / 710)*j*2;
float3 quadFull = tex2D(Ganossa_MF_QuadFullColor, coord).xyz;
float quadFullPow = quadFull.x+quadFull.y+quadFull.z;
#line 113
if(i <   792/2/2 && j <   710/2/2)
quadR.x += quadFullPow;
else if(i >   792/2/2 && j <   710/2/2)
quadR.y += quadFullPow;
else if(i <   792/2/2 && j >   710/2/2)
quadR.z += quadFullPow;
else
...
{
float4 Ganossa_MF_Quad = saturate(tex2D(Ganossa_MF_QuadColor, float2((1.0 / 792), (1.0 / 710)))-0.1f);
#line 130
if (mfDebug)
{
#line 133
