...
{
float rotX = ((tc.x * 2.0 - 1.0) *  (1280 * (1.0 / 720)) * cos(angle)) - ((tc.y * 2.0 - 1.0) * sin(angle));
float rotY = ((tc.y * 2.0 - 1.0) * cos(angle)) + ((tc.x * 2.0 - 1.0) *  (1280 * (1.0 / 720)) * sin(angle));
rotX = ((rotX /  (1280 * (1.0 / 720))) * 0.5 + 0.5);
rotY = rotY * 0.5 + 0.5;
#line 113
...
float3 noise = pnoise3D(float3(rotCoordsR *  float2(1280, 720) / grainsize, 0.0)).xxx;
#line 122
if (coloramount > 0)
{
float2 rotCoordsG = coordRot(texCoord, timer + rotOffset.y);
float2 rotCoordsB = coordRot(texCoord, timer + rotOffset.z);
noise.g = lerp(noise.r, pnoise3D(float3(rotCoordsG *  float2(1280, 720) / grainsize, 1.0)), coloramount);
noise.b = lerp(noise.r, pnoise3D(float3(rotCoordsB *  float2(1280, 720) / grainsize, 2.0)), coloramount);
}
#line 130
