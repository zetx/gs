...
namespace _ {
#line 42
 texture2D tArcaneBloom_Bloom0 { Width  = 1799 / 2; Height = 998 / 2; Format = RGBA16F; };
 texture2D tArcaneBloom_Bloom1 { Width  = 1799 / 4; Height = 998 / 4; Format = RGBA16F; };
 texture2D tArcaneBloom_Bloom2 { Width  = 1799 / 8; Height = 998 / 8; Format = RGBA16F; };
 texture2D tArcaneBloom_Bloom3 { Width  = 1799 / 16; Height = 998 / 16; Format = RGBA16F; };
 texture2D tArcaneBloom_Bloom4 { Width  = 1799 / 32; Height = 998 / 32; Format = RGBA16F; };
#line 52
...
#line 132
static const float AspectRatio = 1799 * (1.0 / 998);
static const float2 PixelSize = float2((1.0 / 1799), (1.0 / 998));
static const float2 ScreenSize = float2(1799, 998);
#line 142
...
#line 363
 texture2D tArcaneBloom_Bloom0Alt { Width  = 1799 / 2; Height = 998 / 2; Format = RGBA16F; }; sampler2D sBloom0Alt { Texture = tArcaneBloom_Bloom0Alt; };
 texture2D tArcaneBloom_Bloom1Alt { Width  = 1799 / 4; Height = 998 / 4; Format = RGBA16F; }; sampler2D sBloom1Alt { Texture = tArcaneBloom_Bloom1Alt; };
 texture2D tArcaneBloom_Bloom2Alt { Width  = 1799 / 8; Height = 998 / 8; Format = RGBA16F; }; sampler2D sBloom2Alt { Texture = tArcaneBloom_Bloom2Alt; };
 texture2D tArcaneBloom_Bloom3Alt { Width  = 1799 / 16; Height = 998 / 16; Format = RGBA16F; }; sampler2D sBloom3Alt { Texture = tArcaneBloom_Bloom3Alt; };
 texture2D tArcaneBloom_Bloom4Alt { Width  = 1799 / 32; Height = 998 / 32; Format = RGBA16F; }; sampler2D sBloom4Alt { Texture = tArcaneBloom_Bloom4Alt; };
#line 372
...
#line 557
  ...
  float4 PS_BlurX_Bloom0( float4 position : SV_POSITION, float2 uv       : TEXCOORD ) : SV_TARGET { const float2 dir = float2((1.0 / 1799) * 2 * 0.5, 0.0); return float4(gaussian_blur(sBloom0, uv, dir), 1.0); }  float4 PS_BlurY_Bloom0Alt( float4 position : SV_POSITION, float2 uv       : TEXCOORD ) : SV_TARGET { const float2 dir = float2(0.0, (1.0 / 998) * 2 * 0.5); return float4(gaussian_blur(sBloom0Alt, uv, dir), 1.0); }
#line 560
  ...
  float4 PS_BlurX_Bloom1( float4 position : SV_POSITION, float2 uv       : TEXCOORD ) : SV_TARGET { const float2 dir = float2((1.0 / 1799) * 4 * 1.0, 0.0); return float4(gaussian_blur(sBloom1, uv, dir), 1.0); }  float4 PS_BlurY_Bloom1Alt( float4 position : SV_POSITION, float2 uv       : TEXCOORD ) : SV_TARGET { const float2 dir = float2(0.0, (1.0 / 998) * 4 * 1.0); return float4(gaussian_blur(sBloom1Alt, uv, dir), 1.0); }
#line 563
  ...
  float4 PS_BlurX_Bloom2( float4 position : SV_POSITION, float2 uv       : TEXCOORD ) : SV_TARGET { const float2 dir = float2((1.0 / 1799) * 8 * 1.0, 0.0); return float4(gaussian_blur(sBloom2, uv, dir), 1.0); }  float4 PS_BlurY_Bloom2Alt( float4 position : SV_POSITION, float2 uv       : TEXCOORD ) : SV_TARGET { const float2 dir = float2(0.0, (1.0 / 998) * 8 * 1.0); return float4(gaussian_blur(sBloom2Alt, uv, dir), 1.0); }
#line 566
...
#line 569
  ...
  float4 PS_BlurX_Bloom4( float4 position : SV_POSITION, float2 uv       : TEXCOORD ) : SV_TARGET { const float2 dir = float2((1.0 / 1799) * 32 * 3.0, 0.0); return float4(gaussian_blur(sBloom4, uv, dir), 1.0); }  float4 PS_BlurY_Bloom4Alt( float4 position : SV_POSITION, float2 uv       : TEXCOORD ) : SV_TARGET { const float2 dir = float2(0.0, (1.0 / 998) * 32 * 3.0); return float4(gaussian_blur(sBloom4Alt, uv, dir), 1.0); }
#line 575
