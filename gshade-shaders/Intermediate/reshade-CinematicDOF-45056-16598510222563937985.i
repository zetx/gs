...
texture texCDCoC				{ Width = 5360; Height = 1440; Format = R16F; };
texture texCDCoCTileTmp			{ Width = 5360/(			1			*		1); Height = 1440/(			1			*		1); Format = R16F; };	
texture texCDCoCTile			{ Width = 5360/(			1			*		1); Height = 1440/(			1			*		1); Format = R16F; };	
texture texCDCoCTileNeighbor	{ Width = 5360/(			1			*		1); Height = 1440/(			1			*		1); Format = R16F; };	
texture texCDCoCTmp1			{ Width = 5360/2; Height = 1440/2; Format = R16F; };	
texture texCDCoCBlurred			{ Width = 5360/2; Height = 1440/2; Format = RG16F; };	
texture texCDBuffer1 			{ Width = 5360/2; Height = 1440/2; Format = RGBA16F; };
texture texCDBuffer2 			{ Width = 5360/2; Height = 1440/2; Format = RGBA16F; };
texture texCDBuffer3 			{ Width = 5360/2; Height = 1440/2; Format = RGBA16F; };
texture texCDBuffer4 			{ Width = 5360; Height = 1440; Format = RGBA16F; }; 	
texture texCDBuffer5 			{ Width = 5360; Height = 1440; Format = RGBA16F; }; 	
...
{
const float tileSize = 			1			 * (float( float2(5360, 1440).x) / 	1920.0f);
float minCoC = 10;
float coc;
float2 coordOffset = float2( float2((1.0 / 5360), (1.0 / 1440)).x, 0);
...
coc = tex2Dlod(source, float4(texcoord - coordOffset, 0, 0)).r;
minCoC = min(minCoC, coc);
coordOffset.x+= float2((1.0 / 5360), (1.0 / 1440)).x;
}
return minCoC;
}
#line 487
...
{
const float tileSize = 			1			 * (float( float2(5360, 1440).y) / 	1200.0f);
float minCoC = 10;
float coc;
float2 coordOffset = float2(0,  float2((1.0 / 5360), (1.0 / 1440)).y);
[unroll]
for(float i = 0; i <= tileSize; ++i)
{
coc = tex2Dlod(source, float4(texcoord + coordOffset, 0, 0)).r;
...
coc = tex2Dlod(source, float4(texcoord - coordOffset, 0, 0)).r;
minCoC = min(minCoC, coc);
coordOffset.y+= float2((1.0 / 5360), (1.0 / 1440)).y;
}
return minCoC;
}
#line 506
...
{
float minCoC = 10;
const float tileSizeX = 			1			 * (float( float2(5360, 1440).x) / 	1920.0f);
const float tileSizeY = 			1			 * (float( float2(5360, 1440).y) / 	1200.0f);
#line 513
const float2 baseCoordOffset = float2( float2((1.0 / 5360), (1.0 / 1440)).x * (tileSizeX*2+1),  float2((1.0 / 5360), (1.0 / 1440)).x * (tileSizeY*2+1));
for(float i=-1;i<2;i++)
{
for(float j=-1;j<2;j++)
{
...
{
float4 toReturn = (radius/2 <= length( float2((1.0 / 5360), (1.0 / 1440)))) && showInFocus ? float4(0.0, 0.0, 1.0, 1.0) : float4(radius, radius, radius, 1.0);
if(radius < 0)
{
...
const float2 ringRadiusDeltaCoords =  float2((1.0 / 5360), (1.0 / 1440)) * (nearPlaneBlurInPixels / (numberOfRings-1));
...
float3 averageGained = AccentuateWhites(average.rgb);
float2 pointOffset = float2(0,0);
const float2 ringRadiusDeltaCoords =  ( float2((1.0 / 5360), (1.0 / 1440)) * blurInfo.farPlaneMaxBlurInPixels * fragmentRadius) / blurInfo.numberOfRings;
float2 currentRingRadiusCoords = ringRadiusDeltaCoords;
...
float2 pointOffset = float2(0,0);
#line 753
const float2 ringRadiusDeltaCoords =  float2((1.0 / 5360), (1.0 / 1440))
...
const float lengthPixelSize = length( float2((1.0 / 5360), (1.0 / 1440)));
...
toFill.pixelSizeLength = length( float2((1.0 / 5360), (1.0 / 1440)));
#line 887
...
float pixelSizeLength = length( float2((1.0 / 5360), (1.0 / 1440)));
...
blurInfo.cocFactorPerPixel = length( float2((1.0 / 5360), (1.0 / 1440))) * blurInfo.farPlaneMaxBlurInPixels;	
return blurInfo;
}
#line 947
void PS_DetermineCurrentFocus(float4 vpos : SV_Position, float2 texcoord : TEXCOORD, out float fragment : SV_Target0)
{
const float2 autoFocusPointToUse = UseMouseDrivenAutoFocus ? MouseCoords *  float2((1.0 / 5360), (1.0 / 1440)) : AutoFocusPoint;
...
float2( float2((1.0 / 5360), (1.0 / 1440)).x * ( float2(5360, 1440).x/	1920.0f), 0.0), true);
}
#line 1016
...
float2(0.0,  float2((1.0 / 5360), (1.0 / 1440)).y * ( float2(5360, 1440).y/	1200.0f)), false),
...
{
const float4 coord =  float2((1.0 / 5360), (1.0 / 1440)).xyxy * float4(1, 1, -1, 0);
float4 average;
...
{
fragment = PerformFullFragmentGaussianBlur(SamplerCDBuffer4, texcoord, float2( float2((1.0 / 5360), (1.0 / 1440)).x, 0.0));
}
#line 1076
...
#line 1105
fragment = PerformFullFragmentGaussianBlur(SamplerCDBuffer5, focusInfo.texcoord, float2(0.0,  float2((1.0 / 5360), (1.0 / 1440)).y));
float4 originalFragment = tex2D(SamplerCDBuffer4, focusInfo.texcoord);
#line 1108
float2 uv = float2(5360, 1440) / float2( 512.0f, 512.0f ); 
...
fragment.rgb = lerp(originalFragment.rgb, fragment.rgb, saturate(coc < length( float2((1.0 / 5360), (1.0 / 1440))) ? 0 : 4 * coc));
fragment.w = 1.0;
#line 1120
...
if(UseAutoFocus)
{
const float2 focusPointCoords = UseMouseDrivenAutoFocus ? MouseCoords *  float2((1.0 / 5360), (1.0 / 1440)) : AutoFocusPoint;
fragment = lerp(fragment, FocusCrosshairColor, FocusCrosshairColor.w * saturate(exp(-5360 * length(focusInfo.texcoord - float2(focusPointCoords.x, focusInfo.texcoord.y)))));
