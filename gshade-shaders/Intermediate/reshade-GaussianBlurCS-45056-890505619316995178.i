...
texture PlaceHolder {Width = 1024; Height = 768; Format = RGB10A2;};
texture Convolution {Width = 1024; Height = 768; Format = RGB10A2;};
#line 57
...
sampleCoord.x += i;
sampleCoord = clamp(sampleCoord, 0, float2(1024, 768));
...
int2 sampleCoord = sampleOffset + groupCoord;
sampleCoord.x += i;
sampleCoord = clamp(sampleCoord, 0, float2(768, 1024));
...
DispatchSizeX =  (int(1024 +  32 *  4 - 1) / int( 32 *  4));
DispatchSizeY =  (int(768 +  2 - 1) / int( 2));
}
pass
{
ComputeShader = VerticalPassCS< 32,  2>;
DispatchSizeX =  (int(768 +  32 *  4 - 1) / int( 32 *  4));
DispatchSizeY =  (int(1024 +  2 - 1) / int( 2));
}
pass
{
