...
texture Luma<pooled = true;>{Width = 5360; Height = 1440; Format = R8;};
texture Temp<pooled = true;>{Width = 5360; Height = 1440; Format = R8;};
#line 101
...
{
float4 r = tex2DgatherR(sBackBuffer, (float2(id.xy * 2) + 0.5)/float2(5360, 1440));
float4 g = tex2DgatherG(sBackBuffer, (float2(id.xy * 2) + 0.5)/float2(5360, 1440));
float4 b = tex2DgatherB(sBackBuffer, (float2(id.xy * 2) + 0.5)/float2(5360, 1440));
#line 176
float4 luma;
...
(sampleCoord > float2(5360, 1440)) ? float2(5360, 1440) - (sampleCoord - float2(5360, 1440)) : sampleCoord;
#line 207
sampleCoord /= float2(5360, 1440);
#line 209
...
DispatchSizeX =  uint(((5360) + (16) - 1) / (16));
DispatchSizeY =  uint(((1440) + (16) - 1) / (16));
}
pass
{
ComputeShader = KuwaharaSharpCS< uint2(8, 8).x,  uint2(8, 8).y>;
DispatchSizeX =  uint(((5360) + (( uint2(1, 1).x *  uint2(8, 8).x)) - 1) / (( uint2(1, 1).x *  uint2(8, 8).x)));
