...
#line 447
texture2D VHS_InputTexA    { Width = 1798; Height = 997; Format = RGBA8; };
texture2D VHS_InputTexB    { Width = 1798; Height = 997; Format = RGBA8; };
texture2D VHS_FeedbackTexA { Width = 1798; Height = 997; Format = RGBA8; };
texture2D VHS_FeedbackTexB { Width = 1798; Height = 997; Format = RGBA8; };
texture2D _TapeTex { Width =  	int(((float)    (float)997	*(float)1798/(float)997)); Height =     (float)997	; Format = RGBA8; };
#line 453
...
#line 706
const float y = p.y* float2(1798, 997).y;
const float s = t*2.0;
...
if (VHS_FishEye_Hyperspace){
#line 769
const float prop =  float2(1798, 997).x /  float2(1798, 997).y;
const float2 m = float2(0.5, 0.5 / prop);
const float2 d = (uv* float2(1798, 997).xy) / float2(1798, 997).x - m;
...
else uv = (uv* float2(1798, 997).xy) / float2(1798, 997).x;
#line 787
uv.y *=  prop;
}
#line 795
...
if(SLN==0.0) SLN =  float2(1798, 997).y;
#line 854
if(SLN_Noise==0 || SLN_Noise>SLN) SLN_Noise = SLN;
#line 856
ONE_X = 1.0/ float2(1798, 997).x; 
ONE_Y = 1.0/ float2(1798, 997).y;
#line 860
if (VHS_Twitch_V){
...
#line 910
const float ScreenLinesNumX = SLN_Noise *  float2(1798, 997).x /  float2(1798, 997).y;
const float SLN_X = noiseQuantizeX*( float2(1798, 997).x - ScreenLinesNumX) + ScreenLinesNumX;
pn.x = floor( pn.x * SLN_X )/SLN_X;
#line 914
const float2 pn_ = pn* float2(1798, 997).xy;
#line 917
...
if(SLN==0.0) SLN =  float2(1798, 997).y;
#line 1101
float ONE_X = 1.0 /  float2(1798, 997).x;  
ONE_X *= bleedAmount; 
#line 1105
if (VHS_FishEye){
...
const float3 sums = 	 t2d( (p - float2( 0.5 *   	float2((1.0 / 1798), (1.0 / 997)).x, .0)) + float2( (offset - float(taps)) * (ONE_X), 0.0)); +
 t2d( (p - float2( 0.5 *   	float2((1.0 / 1798), (1.0 / 997)).x, .0)) + float2( (float(taps) - offset) * (ONE_X), 0.0)); ;
#line 1459
...
#line 1469
signal += t2d( (p - float2( 0.5 *   	float2((1.0 / 1798), (1.0 / 997)).x, .0))) * adj;
norm += adj;
signal = signal / norm;
} else {
#line 1474
...
const float one_x = 1.0/ float2(1798, 997).x;
...
p.y=1-p.y;
#line 1597
const float2 p_ = p* float2(1798, 997);
#line 1599
float ns = 0.0; 
float nt = 0.0; 
float nl = 0.0; 
...
if (VHS_FilmGrain) {
#line 1620
ns += filmGrain((p_-0.5* float2(1798, 997).xy)*0.5, t, filmGrainPower ) * filmGrainAmount;
}
#line 1623
