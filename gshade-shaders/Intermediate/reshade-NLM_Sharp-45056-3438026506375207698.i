...
{
#line 225
const float Up = LI(BB(texcoord, float2( 0,- float2((1.0 / 1309), (1.0 / 762)).y)).rgb);
const float Left = LI(BB(texcoord, float2(- float2((1.0 / 1309), (1.0 / 762)).x, 0)).rgb);
const float Center = LI(BB(texcoord, 0).rgb);
const float Right = LI(BB(texcoord, float2(  float2((1.0 / 1309), (1.0 / 762)).x, 0)).rgb);
const float Down = LI(BB(texcoord, float2( 0,  float2((1.0 / 1309), (1.0 / 762)).y)).rgb);
#line 231
...
#line 242
float sum2;
const float2 RPC_WS =  float2((1.0 / 1309), (1.0 / 762)) * GT();
float4 sum1;
#line 246
for(float y = - 1 ; y <=  1 ; ++y)
{
