...
texture Luma<pooled = true;>{Width = 1309; Height = 762; Format = R8;};
texture Temp<pooled = true;>{Width = 1309; Height = 762; Format = R8;};
#line 101
...
{
float4 r = tex2DgatherR(sBackBuffer, (float2(id.xy * 2) + 0.5)/float2(1309, 762));
float4 g = tex2DgatherG(sBackBuffer, (float2(id.xy * 2) + 0.5)/float2(1309, 762));
float4 b = tex2DgatherB(sBackBuffer, (float2(id.xy * 2) + 0.5)/float2(1309, 762));
#line 176
float4 luma;
...
(sampleCoord > float2(1309, 762)) ? float2(1309, 762) - (sampleCoord - float2(1309, 762)) : sampleCoord;
#line 207
sampleCoord /= float2(1309, 762);
#line 209
...
DispatchSizeX =  uint(((1309) + (16) - 1) / (16));
DispatchSizeY =  uint(((762) + (16) - 1) / (16));
}
pass
{
ComputeShader = KuwaharaSharpCS< uint2(8, 8).x,  uint2(8, 8).y>;
DispatchSizeX =  uint(((1309) + (( uint2(1, 1).x *  uint2(8, 8).x)) - 1) / (( uint2(1, 1).x *  uint2(8, 8).x)));
DispatchSizeY =  uint(((762) + (( uint2(1, 1).y *  uint2(8, 8).y)) - 1) / (( uint2(1, 1).y *  uint2(8, 8).y)));
}
#line 369
pass
{
