...
#line 186
const float2 overlayPos = pos * ( float2(1309, 762) - size);
#line 188
if(all(vpos.xy >= overlayPos) && all(vpos.xy < overlayPos + size))
{
x = MapTwo(texcoord.x, float2(overlayPos.x, overlayPos.x + size.x) / 1309, float2(0.0, 1.0));
y = MapTwo(texcoord.y, float2(overlayPos.y, overlayPos.y + size.y) / 762, float2(0.0, 1.0));
