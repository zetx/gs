...
#line 146
texture VRS {Width =  (uint2( (uint(uint(1799 +  8 - 1) / uint( 8))),  (uint(uint(998 +  8 - 1) / uint( 8))))).x; Height =  (uint2( (uint(uint(1799 +  8 - 1) / uint( 8))),  (uint(uint(998 +  8 - 1) / uint( 8))))).y; Format = RGBA8;};
...
{
return uint(tex2Dfetch(sVRS, int2(texcoord *  (uint2( (uint(uint(1799 +  8 - 1) / uint( 8))),  (uint(uint(998 +  8 - 1) / uint( 8))))))).w * 255);
}
}
#line 184
...
{
float3 variances = (tex2Dfetch(sVRS, int2(texcoord *  (uint2( (uint(uint(1799 +  8 - 1) / uint( 8))),  (uint(uint(998 +  8 - 1) / uint( 8))))))).xyz / 4);
...
int2 grid = uint2(texcoord.xy * float2(1799, 998)) %  8;
...
#line 80
static const int2 g_Resolution = int2(1799, 998);
...
float2 texcoord = input.vPosition.xy * float2((1.0 / 1799), (1.0 / 998));
...
DispatchSizeX =  (uint2( (uint(uint( (uint2( (uint(uint(1799 +  8 - 1) / uint( 8))),  (uint(uint(998 +  8 - 1) / uint( 8))))).x + 2 - 1) / uint(2))),  (uint(uint( (uint2( (uint(uint(1799 +  8 - 1) / uint( 8))),  (uint(uint(998 +  8 - 1) / uint( 8))))).y + 2 - 1) / uint(2))))).x;
DispatchSizeY =  (uint2( (uint(uint( (uint2( (uint(uint(1799 +  8 - 1) / uint( 8))),  (uint(uint(998 +  8 - 1) / uint( 8))))).x + 2 - 1) / uint(2))),  (uint(uint( (uint2( (uint(uint(1799 +  8 - 1) / uint( 8))),  (uint(uint(998 +  8 - 1) / uint( 8))))).y + 2 - 1) / uint(2))))).y;
}
pass
{
VertexShader = mainVS;
