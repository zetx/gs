...
#line 73
texture HistogramRows {Width =  1024; Height =  uint( (int( uint( uint2((792 /  4), (710 /  4)).x *  uint2((792 /  4), (710 /  4)).y) +  128 - 1) / int( 128))); Format = R8;};
#line 75
...
{
float2 coord = floor(float2(id %  uint2((792 /  4), (710 /  4)).x, id /  uint2((792 /  4), (710 /  4)).x)) *  4 + 0.5;
float luma = (round(dot(tex2Dfetch(sBackBuffer, coord).rgb, float3(0.299, 0.587, 0.114)) * ( 1024 - 1)) + 0.5) / float( 1024);
#line 288
pos.x = luma * 2 - 1;
pos.y = (floor(id /  128) + 0.5) / float( uint( (int( uint( uint2((792 /  4), (710 /  4)).x *  uint2((792 /  4), (710 /  4)).y) +  128 - 1) / int( 128)))) * 2 - 1;
pos.z = 0;
pos.w = 1;
}
#line 294
...
uint2 coord = uint2(round(texcoord * float2(( 1024 - 1), 710 * (65536 / ( 1024 *  4 * 0.5 *  4)))));
if(texcoord.x >= 0)
{
...
{
texcoord = float2(1 - texcoord.x, texcoord.y * (float(710) / float(792)));
...
VertexCount =  uint( uint2((792 /  4), (710 /  4)).x *  uint2((792 /  4), (710 /  4)).y);
...
PrimitiveTopology = POINTLIST;
VertexCount =  (int( 1024 *  uint( (int( uint( uint2((792 /  4), (710 /  4)).x *  uint2((792 /  4), (710 /  4)).y) +  128 - 1) / int( 128))) + 8 - 1) / int(8));
ClearRenderTargets = true;
BlendEnable = true;
BlendOp = ADD;
SrcBlend = ONE;
