...
texture Luma<pooled = true;>{Width = 1024; Height = 768; Format = R8;};
texture Temp<pooled = true;>{Width = 1024; Height = 768; Format = R8;};
#line 101
...
{
float4 r = tex2DgatherR(sBackBuffer, (float2(id.xy * 2) + 0.5)/float2(1024, 768));
float4 g = tex2DgatherG(sBackBuffer, (float2(id.xy * 2) + 0.5)/float2(1024, 768));
float4 b = tex2DgatherB(sBackBuffer, (float2(id.xy * 2) + 0.5)/float2(1024, 768));
#line 176
float4 luma;
...
(sampleCoord > float2(1024, 768)) ? float2(1024, 768) - (sampleCoord - float2(1024, 768)) : sampleCoord;
#line 207
sampleCoord /= float2(1024, 768);
#line 209
...
DispatchSizeX =  uint(((1024) + (16) - 1) / (16));
DispatchSizeY =  uint(((768) + (16) - 1) / (16));
}
pass
{
ComputeShader = KuwaharaSharpCS< uint2(8, 8).x,  uint2(8, 8).y>;
DispatchSizeX =  uint(((1024) + (( uint2(1, 1).x *  uint2(8, 8).x)) - 1) / (( uint2(1, 1).x *  uint2(8, 8).x)));
DispatchSizeY =  uint(((768) + (( uint2(1, 1).y *  uint2(8, 8).y)) - 1) / (( uint2(1, 1).y *  uint2(8, 8).y)));
}
#line 369
pass
{
