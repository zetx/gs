...
float4 offset[3],
 sampler tex) {
#line 604
return    tex2DgatherR(tex, texcoord + float4((1.0 / 1024), (1.0 / 768), 1024, 768).xy * float2(-0.5, -0.5)).grb;
#line 611
}
#line 616
...
out float4 offset[3]) {
offset[0] = mad( float4((1.0 / 1024), (1.0 / 768), 1024, 768).xyxy, float4(-1.0, 0.0, 0.0, -1.0), texcoord.xyxy);
offset[1] = mad( float4((1.0 / 1024), (1.0 / 768), 1024, 768).xyxy, float4( 1.0, 0.0, 0.0,  1.0), texcoord.xyxy);
offset[2] = mad( float4((1.0 / 1024), (1.0 / 768), 1024, 768).xyxy, float4(-2.0, 0.0, 0.0, -2.0), texcoord.xyxy);
}
#line 656
void SMAABlendingWeightCalculationVS(float2 texcoord,
out float2 pixcoord,
out float4 offset[3]) {
pixcoord = texcoord *  float4((1.0 / 1024), (1.0 / 768), 1024, 768).zw;
#line 662
offset[0] = mad( float4((1.0 / 1024), (1.0 / 768), 1024, 768).xyxy, float4(-0.25, -0.125,  1.25, -0.125), texcoord.xyxy);
offset[1] = mad( float4((1.0 / 1024), (1.0 / 768), 1024, 768).xyxy, float4(-0.125, -0.25, -0.125,  1.25), texcoord.xyxy);
#line 666
offset[2] = mad( float4((1.0 / 1024), (1.0 / 768), 1024, 768).xxyy,
...
out float4 offset) {
offset = mad( float4((1.0 / 1024), (1.0 / 768), 1024, 768).xyxy, float4( 1.0, 0.0, 0.0,  1.0), texcoord.xyxy);
}
#line 690
...
const float3 t = float3( float4((1.0 / 1024), (1.0 / 768), 1024, 768).xy, 1.0);
...
float4 coord = float4(texcoord, -1.0, 1.0);
coord.x += 0.25 *  float4((1.0 / 1024), (1.0 / 768), 1024, 768).x; 
const float3 t = float3( float4((1.0 / 1024), (1.0 / 768), 1024, 768).xy, 1.0);
while (coord.z < float( MaxSearchStepsDiagonal - 1) &&
coord.w > 0.9) {
coord.xyz = mad(t, float3(dir, 1.0), coord.xyz);
...
if (d.x + d.y > 2.0) { 
#line 1027
const float4 coords = mad(float4(-d.x + 0.25, d.x, d.y, -d.y - 0.25),  float4((1.0 / 1024), (1.0 / 768), 1024, 768).xyxy, texcoord.xyxy);
float4 c;
...
if (d.x + d.y > 2.0) { 
#line 1062
const float4 coords = mad(float4(-d.x, -d.x, d.y, d.y),  float4((1.0 / 1024), (1.0 / 768), 1024, 768).xyxy, texcoord.xyxy);
float4 c;
...
texcoord = mad(-float2(2.0, 0.0),  float4((1.0 / 1024), (1.0 / 768), 1024, 768).xy, texcoord);
}
#line 1127
const float offset = mad(-(255.0 / 127.0), SMAASearchLength( searchTex, e, 0.0), 3.25);
return mad( float4((1.0 / 1024), (1.0 / 768), 1024, 768).x, offset, texcoord.x);
#line 1141
}
#line 1143
...
e.g == 0.0) { 
e =  tex2Dlod(edgesTex, float4(texcoord, texcoord)).rg;
texcoord = mad(-float2(0.0, 2.0),  float4((1.0 / 1024), (1.0 / 768), 1024, 768).xy, texcoord);
}
const float offset = mad(-(255.0 / 127.0), SMAASearchLength( searchTex, e.gr, 0.0), 3.25);
return mad( float4((1.0 / 1024), (1.0 / 768), 1024, 768).y, offset, texcoord.y);
}
#line 1167
...
d.y = coords.z;
#line 1280
d = abs(round(mad( float4((1.0 / 1024), (1.0 / 768), 1024, 768).zz, d, -pixcoord.xx)));
#line 1284
...
d.y = coords.z;
#line 1321
d = abs(round(mad( float4((1.0 / 1024), (1.0 / 768), 1024, 768).ww, d, -pixcoord.yy)));
#line 1325
const float2 sqrt_d = sqrt(d);
#line 1328
...
#line 1382
const float4 blendingCoord = mad(blendingOffset, float4( float4((1.0 / 1024), (1.0 / 768), 1024, 768).xy, - float4((1.0 / 1024), (1.0 / 768), 1024, 768).xy), texcoord.xyxy);
#line 1386
