...
#line 116
texture2D texColorBypass 	{ Width = 1799; 			  Height = 995; 			    Format = RGBA8; MipLevels = 5+		2	;};
texture2D texDistance 		{ Width = 1799; 			  Height = 995;  			    Format = R16F;  MipLevels = 5+		0	;};
texture2D texSurfaceNormal	{ Width = 1799;                           Height = 995; 		            Format = RGBA8; MipLevels = 5+		2	;};
...
{
const float3 offs = float3( float2((1.0 / 1799), (1.0 / 995)).xy,0);
#line 160
...
for(float y = -3; y <= 3; y++)
{
const float2 offsetcoord 	= texcoord.xy + float2(x,y) *  float2((1.0 / 1799), (1.0 / 995)).xy * 3.5;
...
{
blurcoord.xy 	= (2.0 * iStep - 0.5) * orientation * axis *  float2((1.0 / 1799), (1.0 / 995)).xy + texcoord.xy;
...
currentOffset = texcoord.xy + currentVector.xy * float2(1.0, (1799 * (1.0 / 995))) * (iSample + radiusJitter);
#line 352
