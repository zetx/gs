...
texture Luma<pooled = true;>{Width = 1280; Height = 720; Format = R8;};
texture Temp<pooled = true;>{Width = 1280; Height = 720; Format = R8;};
#line 101
...
{
float4 r = tex2DgatherR(sBackBuffer, (float2(id.xy * 2) + 0.5)/float2(1280, 720));
float4 g = tex2DgatherG(sBackBuffer, (float2(id.xy * 2) + 0.5)/float2(1280, 720));
float4 b = tex2DgatherB(sBackBuffer, (float2(id.xy * 2) + 0.5)/float2(1280, 720));
#line 176
float4 luma;
...
(sampleCoord > float2(1280, 720)) ? float2(1280, 720) - (sampleCoord - float2(1280, 720)) : sampleCoord;
#line 207
sampleCoord /= float2(1280, 720);
#line 209
...
DispatchSizeX =  uint(((1280) + (16) - 1) / (16));
DispatchSizeY =  uint(((720) + (16) - 1) / (16));
}
pass
{
ComputeShader = KuwaharaSharpCS< uint2(8, 8).x,  uint2(8, 8).y>;
DispatchSizeX =  uint(((1280) + (( uint2(1, 1).x *  uint2(8, 8).x)) - 1) / (( uint2(1, 1).x *  uint2(8, 8).x)));
DispatchSizeY =  uint(((720) + (( uint2(1, 1).y *  uint2(8, 8).y)) - 1) / (( uint2(1, 1).y *  uint2(8, 8).y)));
}
#line 369
pass
{
