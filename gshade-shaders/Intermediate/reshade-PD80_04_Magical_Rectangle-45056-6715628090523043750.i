...
> = 1.0;
#line 217
texture texMagicRectangle { Width = 1309; Height = 762; Format = RGBA16F; };
#line 220
...
uv.y         = ( id == 1 ) ? 2.0 : 0.0;
uv.xy        -= center.xy;
uv.y         /=  float( 1309 * (1.0 / 762) );
float dim    = ceil( sqrt( 1309 * 1309 + 762 * 762 )); 
float maxlen = min( 1309, 762 );
dim          = dim / maxlen; 
uv.xy        /= dim;
