...
color += tex2Dlod(ReShade::BackBuffer, float4(texcoord + float2(sampleOffsets[i]*gBloomHW *  float2((1.0 / 1309),(1.0 / 762)).x, 0.0), 0.0, 0.0)) * sampleWeights[i];
color += tex2Dlod(ReShade::BackBuffer, float4(texcoord - float2(sampleOffsets[i]*gBloomHW *  float2((1.0 / 1309),(1.0 / 762)).x, 0.0), 0.0, 0.0)) * sampleWeights[i];
}
#line 174
return float4(color.rgb + TriDither(color.rgb, texcoord, 8), color.a);
...
float4 color = tex2D(ReShade::BackBuffer, texcoord) * sampleWeights[0];
for(int i = 1; i < gN_PASSES; ++i) {
color += tex2Dlod(ReShade::BackBuffer, float4(texcoord + float2(0.0, sampleOffsets[i]*gBloomVW *  float2((1.0 / 1309),(1.0 / 762)).y), 0.0, 0.0)) * sampleWeights[i];
color += tex2Dlod(ReShade::BackBuffer, float4(texcoord - float2(0.0, sampleOffsets[i]*gBloomVW *  float2((1.0 / 1309),(1.0 / 762)).y), 0.0, 0.0)) * sampleWeights[i];
}
#line 196
return float4(color.rgb + TriDither(color.rgb, texcoord, 8), color.a);
...
float4 color = tex2D(ReShade::BackBuffer, texcoord) * sampleWeights[0];
for(int i = 1; i < gN_PASSES; ++i) {
color += tex2Dlod(ReShade::BackBuffer, float4(texcoord + float2(sampleOffsets[i]*gBloomSW *  float2((1.0 / 1309),(1.0 / 762)).x, sampleOffsets[i] *  float2((1.0 / 1309),(1.0 / 762)).y), 0.0, 0.0)) * sampleWeights[i];
color += tex2Dlod(ReShade::BackBuffer, float4(texcoord - float2(sampleOffsets[i]*gBloomSW *  float2((1.0 / 1309),(1.0 / 762)).x, sampleOffsets[i] *  float2((1.0 / 1309),(1.0 / 762)).y), 0.0, 0.0)) * sampleWeights[i];
color += tex2Dlod(ReShade::BackBuffer, float4(texcoord + float2(-sampleOffsets[i]*gBloomSW *  float2((1.0 / 1309),(1.0 / 762)).x, sampleOffsets[i] *  float2((1.0 / 1309),(1.0 / 762)).y), 0.0, 0.0)) * sampleWeights[i];
color += tex2Dlod(ReShade::BackBuffer, float4(texcoord + float2(sampleOffsets[i]*gBloomSW *  float2((1.0 / 1309),(1.0 / 762)).x, -sampleOffsets[i] *  float2((1.0 / 1309),(1.0 / 762)).y), 0.0, 0.0)) * sampleWeights[i];
}
