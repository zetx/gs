...
#line 361
texture Texture1GCRT{Width = 1799; Height = 995; Format = RGBA16F;};
sampler Sampler1GCRT{Texture = Texture1GCRT; MinFilter = Linear; MagFilter = Linear;};
#line 364
texture Texture2GCRT{Width = 1799; Height = 995; Format = RGBA16F;};
...
coord = min(coord, 1.0 - coord) * float2(1.0,  float4( float2(1799, 995), 1.0 /  float2(1799, 995)).y /  float4( float2(1799, 995), 1.0 /  float2(1799, 995)).x);
...
if(IOS > 0.0)
{
const float2 ofactor =  float4( float2(1799, 995), 1.0 /  float2(1799, 995)).xy /  float2( 320,  240).xy;
