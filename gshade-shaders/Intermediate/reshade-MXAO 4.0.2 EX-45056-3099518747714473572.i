...
#line 171
texture2D MXAO_ColorTex 	{ Width = 792;   Height = 710;   Format = RGBA8; MipLevels = 3+		2	;};
texture2D MXAO_DepthTex 	{ Width = 792;   Height = 710;   Format = R16F;  MipLevels = 3+		0	;};
texture2D MXAO_NormalTex	{ Width = 792;   Height = 710;   Format = RGBA8; MipLevels = 3+		2	;};
texture2D MXAO_CullingTex	{ Width = 792/8; Height = 710/8; Format = R8; };
#line 176
...
{
const float2 sampleCoord = MXAO.texcoord.xy + blurOffsets[iStep] *  float2((1.0 / 792), (1.0 / 710)) * radius / inputscale;
#line 303
...
{
const float2 searchRadiusScaled = 0.018 / P.z * float2(1.0, (792 * (1.0 / 710)));
...
{
const float4 cOffsets = float4( float2((1.0 / 792), (1.0 / 710)),- float2((1.0 / 792), (1.0 / 710))) * 8;
...
{
const float3 offs = float3( float2((1.0 / 792), (1.0 / 710)),0);
#line 405
...
{
sampleUV = MXAO.scaledcoord.xy + Dir.xy * float2(1.0,  (792 * (1.0 / 710))) * (iSample + randStep);
...
|| 0.25 * 0.5 * MXAO_SAMPLE_RADIUS / (tex2D(sMXAO_DepthTex,MXAO.scaledcoord.xy).x + 2.0) * 710 < 1.0
