...
> = 1.0;
#line 427
texture texHDR1 { Width = 5360 *  0.6; Height = 1440 *  0.6; Format = RGBA8; };
texture texHDR2 { Width = 5360 *  0.6; Height = 1440 *  0.6; Format = RGBA8; };
...
focusPoint = MouseCoords *  float2((1.0 / 5360), (1.0 / 1440));
else
...
discRadius *= 1.0;
#line 510
hdr2R.x = tex2Dlod(SamplerHDR1, float4(texcoord + float2( 0.000,  1.0) * fRingDOFFringe * discRadius *  float2((1.0 / 5360), (1.0 / 1440)), 0, 0)).x;
hdr2R.y = tex2Dlod(SamplerHDR1, float4(texcoord + float2(-0.866, -0.5) * fRingDOFFringe * discRadius *  float2((1.0 / 5360), (1.0 / 1440)), 0, 0)).y;
hdr2R.z = tex2Dlod(SamplerHDR1, float4(texcoord + float2( 0.866, -0.5) * fRingDOFFringe * discRadius *  float2((1.0 / 5360), (1.0 / 1440)), 0, 0)).z;
#line 514
hdr2R.w = centerDepth;
}
...
float4 tap = tex2Dlod(SamplerHDR2, float4(texcoord + sampleoffset *  float2((1.0 / 5360), (1.0 / 1440)) * discRadius * g / iRingDOFRings, 0, 0));
#line 554
...
{
float4 tap = tex2Dlod(SamplerHDR1, float4(texcoord + (float2(1, 0) * i) * discRadius *  float2((1.0 / 5360), (1.0 / 1440)).x / iMagicDOFBlurQuality, 0, 0));
if (tap.w >= centerDepth*0.99)
tap.w = 1.0;
else
tap.w = pow(abs(tap.w * 2.0 - 1.0), 4.0);
...
#line 637
blurcolor.xyz += pow(abs(min(tex2Dlod(SamplerHDR2, float4(texcoord + tapoffset1 * discRadius *  float2((1.0 / 5360), (1.0 / 1440)) / iMagicDOFBlurQuality, 0, 0)).xyz, tex2Dlod(SamplerHDR2, float4(texcoord + float2(-tapoffset1.x, tapoffset1.y) * discRadius *  float2((1.0 / 5360), (1.0 / 1440)) / iMagicDOFBlurQuality, 0, 0)).xyz)), fMagicDOFColorCurve);
blurcolor.w += 1.0;
}
#line 641
blurcolor.xyz /= blurcolor.w;
...
#line 665
float4 chroma1 = tex2D(SamplerHDR1, texcoord + discRadius *  float2((1.0 / 5360), (1.0 / 1440)) * distortion.x);
chroma1.w = smoothstep(0.0, centerDepth, chroma1.w);
hdr2R.x = lerp(hdr2R.x, chroma1.x, chroma1.w);
#line 669
float4 chroma2 = tex2D(SamplerHDR1, texcoord + discRadius *  float2((1.0 / 5360), (1.0 / 1440)) * distortion.z);
...
#line 770
float4 tap = tex2Dlod(SamplerHDR2, float4(texcoord + sampleOffset * discRadius *  float2((1.0 / 5360), (1.0 / 1440)) / iGPDOFQuality, 0, 0));
#line 772
...
#line 826
const float2 tcoord = coord.xy + (float)i * taxis * discRadius *  float2((1.0 / 5360), (1.0 / 1440)) * 0.5 / iMatsoDOFBokehQuality;
#line 828
float4 ct;
if (bMatsoDOFChromaEnable)
ct = GetMatsoDOFCA(SamplerHDRX, tcoord.xy, discRadius *  float2((1.0 / 5360), (1.0 / 1440)).x * 0.5 / iMatsoDOFBokehQuality);
else
...
float2 discRadius = CoC *  float2((1.0 / 5360), (1.0 / 1440));
int shapeVertices;
...
scenecolor.xyz += tex2Dlod(SamplerHDR2, float4((coord.xy * (1.0 + ((1.0 / 5360) * centerfact * discRadius) * ((c + 0.5) / iADOF_ImageChromaHues - 0.5)) * 0.5 + 0.5) *  0.6, 0, 0)).xyz*tempchroma.xyz;
...
#line 1338
const float2 blurmult = smoothstep(0.3, 0.8, abs(scenecolor.w * 2.0 - 1.0)) *  float2((1.0 / 5360), (1.0 / 1440)) * fADOF_SmootheningAmount;
#line 1340
