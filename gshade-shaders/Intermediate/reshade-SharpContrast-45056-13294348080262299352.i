...
texture Luma<pooled = true;>{Width = 1798; Height = 997; Format = R8;};
texture Temp<pooled = true;>{Width = 1798; Height = 997; Format = R8;};
#line 101
...
{
float4 r = tex2DgatherR(sBackBuffer, (float2(id.xy * 2) + 0.5)/float2(1798, 997));
float4 g = tex2DgatherG(sBackBuffer, (float2(id.xy * 2) + 0.5)/float2(1798, 997));
float4 b = tex2DgatherB(sBackBuffer, (float2(id.xy * 2) + 0.5)/float2(1798, 997));
#line 176
float4 luma;
...
(sampleCoord > float2(1798, 997)) ? float2(1798, 997) - (sampleCoord - float2(1798, 997)) : sampleCoord;
#line 207
sampleCoord /= float2(1798, 997);
#line 209
...
DispatchSizeX =  uint(((1798) + (16) - 1) / (16));
DispatchSizeY =  uint(((997) + (16) - 1) / (16));
}
pass
{
ComputeShader = KuwaharaSharpCS< uint2(8, 8).x,  uint2(8, 8).y>;
DispatchSizeX =  uint(((1798) + (( uint2(1, 1).x *  uint2(8, 8).x)) - 1) / (( uint2(1, 1).x *  uint2(8, 8).x)));
DispatchSizeY =  uint(((997) + (( uint2(1, 1).y *  uint2(8, 8).y)) - 1) / (( uint2(1, 1).y *  uint2(8, 8).y)));
}
#line 369
pass
{
