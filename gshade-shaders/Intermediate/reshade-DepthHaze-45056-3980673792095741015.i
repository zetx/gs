...
#line 44
texture   Otis_FragmentBuffer1 	{ Width = 792; Height = 710; Format = RGBA8;};
texture   Otis_FragmentBuffer2 	< pooled = true; > { Width = 792; Height = 710; Format = RGBA8;};
#line 50
...
for(float i = 1; i < 5; ++i)
{
float2 sourceCoords = texcoord + float2(i *  float2((1.0 / 792), (1.0 / 710)).x, 0.0);
...
n+=weight;
#line 75
sourceCoords = texcoord - float2(i *  float2((1.0 / 792), (1.0 / 710)).x, 0.0);
...
for(float j = 1; j < 5; ++j)
{
float2 sourceCoords = texcoord + float2(0.0, j *  float2((1.0 / 792), (1.0 / 710)).y);
...
n+=weight;
#line 97
sourceCoords = texcoord - float2(0.0, j *  float2((1.0 / 792), (1.0 / 710)).y);
