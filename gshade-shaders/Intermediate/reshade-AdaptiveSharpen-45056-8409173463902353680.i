...
#line 117
texture AS_Pass0Tex < pooled = true; > { Width = 1024; Height = 768; Format = RG16F; };
...
{
#line 224
const float3 c[13] = {       ( saturate(tex2D(ReShade::BackBuffer,       (  float2((1.0 / 1024), (1.0 / 768))*float2(0, 0) + tex )).rgb) ),       ( saturate(tex2D(ReShade::BackBuffer,       (  float2((1.0 / 1024), (1.0 / 768))*float2(-1, -1) + tex )).rgb) ),       ( saturate(tex2D(ReShade::BackBuffer,       (  float2((1.0 / 1024), (1.0 / 768))*float2(0, -1) + tex )).rgb) ),       ( saturate(tex2D(ReShade::BackBuffer,       (  float2((1.0 / 1024), (1.0 / 768))*float2(1, -1) + tex )).rgb) ),       ( saturate(tex2D(ReShade::BackBuffer,       (  float2((1.0 / 1024), (1.0 / 768))*float2(-1, 0) + tex )).rgb) ),
      ( saturate(tex2D(ReShade::BackBuffer,       (  float2((1.0 / 1024), (1.0 / 768))*float2(1, 0) + tex )).rgb) ),       ( saturate(tex2D(ReShade::BackBuffer,       (  float2((1.0 / 1024), (1.0 / 768))*float2(-1, 1) + tex )).rgb) ),       ( saturate(tex2D(ReShade::BackBuffer,       (  float2((1.0 / 1024), (1.0 / 768))*float2(0, 1) + tex )).rgb) ),       ( saturate(tex2D(ReShade::BackBuffer,       (  float2((1.0 / 1024), (1.0 / 768))*float2(1, 1) + tex )).rgb) ),       ( saturate(tex2D(ReShade::BackBuffer,       (  float2((1.0 / 1024), (1.0 / 768))*float2(0, -2) + tex )).rgb) ),
      ( saturate(tex2D(ReShade::BackBuffer,       (  float2((1.0 / 1024), (1.0 / 768))*float2(-2, 0) + tex )).rgb) ),       ( saturate(tex2D(ReShade::BackBuffer,       (  float2((1.0 / 1024), (1.0 / 768))*float2(2, 0) + tex )).rgb) ),       ( saturate(tex2D(ReShade::BackBuffer,       (  float2((1.0 / 1024), (1.0 / 768))*float2(0, 2) + tex )).rgb) ) };
#line 229
...
{
float3 origsat =       ( saturate(tex2D(ReShade::BackBuffer,       (  float2((1.0 / 1024), (1.0 / 768))*float2(0, 0) + tex )).rgb) );
#line 270
const float2 d[25] = {       ( tex2D(AS_Pass0Sampler,       (  float2((1.0 / 1024), (1.0 / 768))*float2(0, 0) + tex )).xy ),       ( tex2D(AS_Pass0Sampler,       (  float2((1.0 / 1024), (1.0 / 768))*float2(-1, -1) + tex )).xy ),       ( tex2D(AS_Pass0Sampler,       (  float2((1.0 / 1024), (1.0 / 768))*float2(0, -1) + tex )).xy ),       ( tex2D(AS_Pass0Sampler,       (  float2((1.0 / 1024), (1.0 / 768))*float2(1, -1) + tex )).xy ),       ( tex2D(AS_Pass0Sampler,       (  float2((1.0 / 1024), (1.0 / 768))*float2(-1, 0) + tex )).xy ),
      ( tex2D(AS_Pass0Sampler,       (  float2((1.0 / 1024), (1.0 / 768))*float2(1, 0) + tex )).xy ),       ( tex2D(AS_Pass0Sampler,       (  float2((1.0 / 1024), (1.0 / 768))*float2(-1, 1) + tex )).xy ),       ( tex2D(AS_Pass0Sampler,       (  float2((1.0 / 1024), (1.0 / 768))*float2(0, 1) + tex )).xy ),       ( tex2D(AS_Pass0Sampler,       (  float2((1.0 / 1024), (1.0 / 768))*float2(1, 1) + tex )).xy ),       ( tex2D(AS_Pass0Sampler,       (  float2((1.0 / 1024), (1.0 / 768))*float2(0, -2) + tex )).xy ),
      ( tex2D(AS_Pass0Sampler,       (  float2((1.0 / 1024), (1.0 / 768))*float2(-2, 0) + tex )).xy ),       ( tex2D(AS_Pass0Sampler,       (  float2((1.0 / 1024), (1.0 / 768))*float2(2, 0) + tex )).xy ),       ( tex2D(AS_Pass0Sampler,       (  float2((1.0 / 1024), (1.0 / 768))*float2(0, 2) + tex )).xy ),       ( tex2D(AS_Pass0Sampler,       (  float2((1.0 / 1024), (1.0 / 768))*float2(0, 3) + tex )).xy ),       ( tex2D(AS_Pass0Sampler,       (  float2((1.0 / 1024), (1.0 / 768))*float2(1, 2) + tex )).xy ),
      ( tex2D(AS_Pass0Sampler,       (  float2((1.0 / 1024), (1.0 / 768))*float2(-1, 2) + tex )).xy ),       ( tex2D(AS_Pass0Sampler,       (  float2((1.0 / 1024), (1.0 / 768))*float2(3, 0) + tex )).xy ),       ( tex2D(AS_Pass0Sampler,       (  float2((1.0 / 1024), (1.0 / 768))*float2(2, 1) + tex )).xy ),       ( tex2D(AS_Pass0Sampler,       (  float2((1.0 / 1024), (1.0 / 768))*float2(2, -1) + tex )).xy ),       ( tex2D(AS_Pass0Sampler,       (  float2((1.0 / 1024), (1.0 / 768))*float2(-3, 0) + tex )).xy ),
      ( tex2D(AS_Pass0Sampler,       (  float2((1.0 / 1024), (1.0 / 768))*float2(-2, 1) + tex )).xy ),       ( tex2D(AS_Pass0Sampler,       (  float2((1.0 / 1024), (1.0 / 768))*float2(-2, -1) + tex )).xy ),       ( tex2D(AS_Pass0Sampler,       (  float2((1.0 / 1024), (1.0 / 768))*float2(0, -3) + tex )).xy ),       ( tex2D(AS_Pass0Sampler,       (  float2((1.0 / 1024), (1.0 / 768))*float2(1, -2) + tex )).xy ),       ( tex2D(AS_Pass0Sampler,       (  float2((1.0 / 1024), (1.0 / 768))*float2(-1, -2) + tex )).xy ) };
#line 279
