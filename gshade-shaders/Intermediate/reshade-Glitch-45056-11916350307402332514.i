...
{
return tex2D(samp, uv - fmod(uv, float2(pixelSize,pixelSize) /  float2(5360, 1440)));
}
#line 72
float rand(float n)
{
...
{
const float dx = sampleSize * (1.0 / 5360);
...
{
const float2 pixel =  float2((1.0 / 5360), (1.0 / 1440));
...
const float shifty = noise(float2(quantize(uv.x + 11.5, 5360 /  16.0) * Timer*0.001, frac(Timer*0.001) * 100.0));
...
float Amount;
#line 130
const float2 texcoord = fragCoord *  float2(5360, 1440); 
const float2 uv = texcoord.xy /  float2(5360, 1440);
if (bUseUV) {
Amount = uv.x; 
}
