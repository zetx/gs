...
#line 151
texture texGravitySeedMap{ Width =  1024/ 1; Height =  768; Format = R16F; };
texture texGravitySeedMapCopy{ Width =  1024/ 1; Height =  768; Format = R16F; };
...
texture texGravityMain{ Width =  1024/ 1; Height =  768; Format = RGBA8; };
texture texGravityDepth{ Width =  1024/ 1; Height =  768; Format = R32F; };
#line 162
...
#line 286
finalist[y] = y;
depthlist[y] = depthlistU[y] = ReShade::GetLinearizedDepth(float2((id.x* 1 + 0.5) / 1024, (y *  768/ 768 + 0.5) / 768));
}
...
#line 295
float4 rgbval =  tex2Dfetch(ReShade::BackBuffer, int2(id.x* 1, yi* 768/ 768), 0); 
float scenedepth = depthlist[yi]; 
float strength = tex2Dfetch(SamplerGravitySeedMap, int2(id.x, yi), 0).r; 
strength = strength * GravityIntensity * inFocus(rgbval, scenedepth, float2((id.x* 1 + 0.5) / 1024, (yi *  768/ 768 + 0.5) / 768)); 
...
{
if (y != finalist[y]) {
float4 storeVal = tex2Dfetch(ReShade::BackBuffer, int2(id.x, finalist[y] *  768/ 768), 0); 
...
pass GravityMain { ComputeShader = gravityMain<1, 1>; DispatchSizeX =  1024/ 1; DispatchSizeY = 1; }
