...
> = 1.0;
#line 217
texture texMagicRectangle { Width = 1799; Height = 995; Format = RGBA16F; };
#line 220
...
uv.y         = ( id == 1 ) ? 2.0 : 0.0;
uv.xy        -= center.xy;
uv.y         /=  float( 1799 * (1.0 / 995) );
float dim    = ceil( sqrt( 1799 * 1799 + 995 * 995 )); 
float maxlen = min( 1799, 995 );
dim          = dim / maxlen; 
uv.xy        /= dim;
