...
#line 832
texture texDMVR  { Width = 3440; Height = 1440; Format = RGBA16F; MipLevels = 6;};
#line 834
sampler SamplerDMVR
{
Texture = texDMVR;
...
#line 839
texture texzBufferVR_P  { Width = 3440; Height = 1440; Format = RG16F; };
#line 841
...
#line 853
texture texzBufferVR_L  { Width = 3440; Height = 1440; Format = R16F; MipLevels = 2; };
#line 855
...
#line 869
texture texPBVR  { Width = 3440; Height = 1440; Format = R8; };
#line 871
...
#line 902
texture LeftTex  { Width = 3440; Height = 1440; Format =  RGB10A2; };
#line 904
sampler SamplerLeft
{
Texture = LeftTex;
...
#line 912
texture RightTex  { Width = 3440; Height = 1440; Format =  RGB10A2; };
#line 914
...
#line 924
texture texLumVR {Width = 3440 *  160 / 3440; Height = 1440 *  160 / 3440; Format = RGBA16F; MipLevels = 8;};
#line 926
...
#line 931
texture texOtherVR {Width = 3440 *  160 / 3440; Height = 1440 *  160 / 3440; Format = R16F; MipLevels = 8;};
#line 933
...
float2 MousecoordsXY = Mousecoords *  float2((1.0 / 3440), (1.0 / 1440)), center = texcoord, Screen_Ratio = float2(1.75,1.0), Size_Color = float2(1+Cursor_SC.x,Cursor_SC.y);
...
#line 1190
texcoord.xy -= DLSS_FSR_Offset.x *  float2((1.0 / 3440), (1.0 / 1440));
#line 1192
...
#line 1326
if(texcoord.x <  float2((1.0 / 3440), (1.0 / 1440)).x * 2 && texcoord.y <  float2((1.0 / 3440), (1.0 / 1440)).y * 2)
R = Fade_in_out(texcoord);
if(1-texcoord.x <  float2((1.0 / 3440), (1.0 / 1440)).x * 2 && 1-texcoord.y <  float2((1.0 / 3440), (1.0 / 1440)).y * 2)
R = Fade(texcoord).x;
if(texcoord.x <  float2((1.0 / 3440), (1.0 / 1440)).x * 2 && 1-texcoord.y <  float2((1.0 / 3440), (1.0 / 1440)).y * 2)
R = Fade(texcoord).y;
if(1-texcoord.x <  float2((1.0 / 3440), (1.0 / 1440)).x * 2 && texcoord.y <  float2((1.0 / 3440), (1.0 / 1440)).y * 2)
...
{   float MS = Diverge *  float2((1.0 / 3440), (1.0 / 1440)).x, GetDepth = smoothstep(0,1,tex2Dlod(SamplerzBufferVR_P, float4(Coordinates,0, 0) ).y),
Details = 0.5,
Perf = Performance_LvL[Performance_Level].x;
float2 ParallaxCoord = Coordinates, CBxy = floor( float2(Coordinates.x * 3440, Coordinates.y * 1440));
#line 1499
if( View_Mode == 1)
...
Offset_Switch = View_Mode > 0 ? 0.0 : 1.0, DB_Offset = D * TP *  float2((1.0 / 3440), (1.0 / 1440)).x;
#line 1525
[loop] 
...
#line 1556
if(Diverge < 0)
ParallaxCoord.x += DepthDiffrence * 1.5 *  float2((1.0 / 3440), (1.0 / 1440)).x;
else
ParallaxCoord.x -= DepthDiffrence * 1.5 *  float2((1.0 / 3440), (1.0 / 1440)).x;
}
...
#line 1599
float fov = FoV-(FoV*0.2), F = -fov + 1,HA = (F - 1)*(3440*0.5)* float2((1.0 / 3440), (1.0 / 1440)).x;
#line 1601
...
float midW = (X - 1)*(3440*0.5)* float2((1.0 / 3440), (1.0 / 1440)).x;
float midH = (Y - 1)*(1440*0.5)* float2((1.0 / 3440), (1.0 / 1440)).y;
#line 1613
texcoord = float2((texcoord.x*X)-midW,(texcoord.y*Y)-midH);
#line 1615
float2 TCL = texcoord,TCR = texcoord;
#line 1617
TCL.x -=  IPD *  float2((1.0 / 3440), (1.0 / 1440)).x*0.5f;
TCR.x +=  IPD *  float2((1.0 / 3440), (1.0 / 1440)).x*0.5f;
#line 1620
float D = Divergence;
#line 1622
...
discard;
#line 1665
float2 C_A = float2(1.0f,1.1375f), midHV = (C_A-1) * float2(3440 * 0.5,1440 * 0.5) *  float2((1.0 / 3440), (1.0 / 1440));
#line 1667
...
{
float2 gridxy = floor(float2(texcoord.x * 3440, texcoord.y * 1440)), TCL = float2(texcoord.x * 2,texcoord.y), TCR = float2(texcoord.x * 2 - 1,texcoord.y), uv_redL, uv_greenL, uv_blueL, uv_redR, uv_greenR, uv_blueR;
...
#line 1926
float2 ScreenPos = float2(1-texcoord.x,1-texcoord.y) *  float2(3440, 1440);
float Debug_Y = 1.0;
...
float2 tex_offset =  float2((1.0 / 3440), (1.0 / 1440)); 
...
float2 RPC_WS =  float2((1.0 / 3440), (1.0 / 1440)) * 1.5;
float Z, factor;
#line 2286
[loop]
