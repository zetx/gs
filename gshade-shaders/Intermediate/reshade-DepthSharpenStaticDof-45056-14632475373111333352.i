...
return ori;
#line 120
const float depthTL = ReShade::GetLinearizedDepth( tex + float2(- float2((1.0 / 1281), (1.0 / 721)).x, - float2((1.0 / 1281), (1.0 / 721)).y) ).r;
const float depthTR = ReShade::GetLinearizedDepth( tex + float2( float2((1.0 / 1281), (1.0 / 721)).x, - float2((1.0 / 1281), (1.0 / 721)).y) ).r;
...
{
#line 164
blur_ori  = tex2D(ReShade::BackBuffer, tex + ( float2((1.0 / 1281), (1.0 / 721)) / 3.0) * offset_bias).rgb;  
blur_ori += tex2D(ReShade::BackBuffer, tex + (- float2((1.0 / 1281), (1.0 / 721)) / 3.0) * offset_bias).rgb; 
#line 167
blur_ori /= 2;  
#line 169
...
{
#line 180
blur_ori  = tex2D(ReShade::BackBuffer, tex + float2( float2((1.0 / 1281), (1.0 / 721)).x, - float2((1.0 / 1281), (1.0 / 721)).y) * 0.5 * offset_bias).rgb; 
blur_ori += tex2D(ReShade::BackBuffer, tex -  float2((1.0 / 1281), (1.0 / 721)) * 0.5 * offset_bias).rgb;  
blur_ori += tex2D(ReShade::BackBuffer, tex +  float2((1.0 / 1281), (1.0 / 721)) * 0.5 * offset_bias).rgb; 
blur_ori += tex2D(ReShade::BackBuffer, tex - float2( float2((1.0 / 1281), (1.0 / 721)).x, - float2((1.0 / 1281), (1.0 / 721)).y) * 0.5 * offset_bias).rgb; 
#line 185
blur_ori *= 0.25;  
...
{
#line 198
blur_ori  = tex2D(ReShade::BackBuffer, tex +  float2((1.0 / 1281), (1.0 / 721)) * float2(0.4, -1.2) * offset_bias).rgb;  
blur_ori += tex2D(ReShade::BackBuffer, tex -  float2((1.0 / 1281), (1.0 / 721)) * float2(1.2, 0.4) * offset_bias).rgb; 
blur_ori += tex2D(ReShade::BackBuffer, tex +  float2((1.0 / 1281), (1.0 / 721)) * float2(1.2, 0.4) * offset_bias).rgb; 
blur_ori += tex2D(ReShade::BackBuffer, tex -  float2((1.0 / 1281), (1.0 / 721)) * float2(0.4, -1.2) * offset_bias).rgb; 
#line 203
blur_ori *= 0.25;  
...
{
#line 216
blur_ori  = tex2D(ReShade::BackBuffer, tex + float2(0.5 *  float2((1.0 / 1281), (1.0 / 721)).x, - float2((1.0 / 1281), (1.0 / 721)).y * offset_bias)).rgb;  
blur_ori += tex2D(ReShade::BackBuffer, tex + float2(offset_bias * - float2((1.0 / 1281), (1.0 / 721)).x, 0.5 * - float2((1.0 / 1281), (1.0 / 721)).y)).rgb; 
blur_ori += tex2D(ReShade::BackBuffer, tex + float2(offset_bias *  float2((1.0 / 1281), (1.0 / 721)).x, 0.5 *  float2((1.0 / 1281), (1.0 / 721)).y)).rgb; 
blur_ori += tex2D(ReShade::BackBuffer, tex + float2(0.5 * - float2((1.0 / 1281), (1.0 / 721)).x,  float2((1.0 / 1281), (1.0 / 721)).y * offset_bias)).rgb; 
#line 221
blur_ori /= 4.0;  
