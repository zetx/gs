...
texture PlaceHolder {Width = 1281; Height = 721; Format = RGB10A2;};
texture Convolution {Width = 1281; Height = 721; Format = RGB10A2;};
#line 57
...
sampleCoord.x += i;
sampleCoord = clamp(sampleCoord, 0, float2(1281, 721));
...
int2 sampleCoord = sampleOffset + groupCoord;
sampleCoord.x += i;
sampleCoord = clamp(sampleCoord, 0, float2(721, 1281));
...
DispatchSizeX =  (int(1281 +  32 *  4 - 1) / int( 32 *  4));
DispatchSizeY =  (int(721 +  2 - 1) / int( 2));
}
pass
{
ComputeShader = VerticalPassCS< 32,  2>;
DispatchSizeX =  (int(721 +  32 *  4 - 1) / int( 32 *  4));
DispatchSizeY =  (int(1281 +  2 - 1) / int( 2));
}
pass
{
