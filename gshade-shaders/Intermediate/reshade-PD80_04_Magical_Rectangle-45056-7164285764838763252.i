...
> = 1.0;
#line 217
texture texMagicRectangle { Width = 1280; Height = 720; Format = RGBA16F; };
#line 220
...
uv.y         = ( id == 1 ) ? 2.0 : 0.0;
uv.xy        -= center.xy;
uv.y         /=  float( 1280 * (1.0 / 720) );
float dim    = ceil( sqrt( 1280 * 1280 + 720 * 720 )); 
float maxlen = min( 1280, 720 );
dim          = dim / maxlen; 
uv.xy        /= dim;
