...
#line 31
texture GaussianBlurTex < pooled = true; > { Width = 1799; Height = 998; Format = RGBA8; };
...
for(int i = 1; i < 4; ++i)
{
color += tex2Dlod(GaussianBlurSampler, float4(texcoord + float2(0.0, offset[i] *  float2((1.0 / 1799), (1.0 / 998)).y) * GaussianBlurOffset, 0.0, 0.0)).rgb * weight[i];
color += tex2Dlod(GaussianBlurSampler, float4(texcoord - float2(0.0, offset[i] *  float2((1.0 / 1799), (1.0 / 998)).y) * GaussianBlurOffset, 0.0, 0.0)).rgb * weight[i];
}
}
#line 54
...
for(int i = 1; i < 4; ++i)
{
color += tex2Dlod(ReShade::BackBuffer, float4(texcoord + float2(offset[i] *  float2((1.0 / 1799), (1.0 / 998)).x, 0.0) * GaussianBlurOffset, 0.0, 0.0)).rgb * weight[i];
color += tex2Dlod(ReShade::BackBuffer, float4(texcoord - float2(offset[i] *  float2((1.0 / 1799), (1.0 / 998)).x, 0.0) * GaussianBlurOffset, 0.0, 0.0)).rgb * weight[i];
}
}
#line 144
