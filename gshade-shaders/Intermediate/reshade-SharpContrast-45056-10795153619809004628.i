...
texture Luma<pooled = true;>{Width = 792; Height = 710; Format = R8;};
texture Temp<pooled = true;>{Width = 792; Height = 710; Format = R8;};
#line 101
...
{
float4 r = tex2DgatherR(sBackBuffer, (float2(id.xy * 2) + 0.5)/float2(792, 710));
float4 g = tex2DgatherG(sBackBuffer, (float2(id.xy * 2) + 0.5)/float2(792, 710));
float4 b = tex2DgatherB(sBackBuffer, (float2(id.xy * 2) + 0.5)/float2(792, 710));
#line 176
float4 luma;
...
(sampleCoord > float2(792, 710)) ? float2(792, 710) - (sampleCoord - float2(792, 710)) : sampleCoord;
#line 207
sampleCoord /= float2(792, 710);
#line 209
...
DispatchSizeX =  uint(((792) + (16) - 1) / (16));
DispatchSizeY =  uint(((710) + (16) - 1) / (16));
}
pass
{
ComputeShader = KuwaharaSharpCS< uint2(8, 8).x,  uint2(8, 8).y>;
DispatchSizeX =  uint(((792) + (( uint2(1, 1).x *  uint2(8, 8).x)) - 1) / (( uint2(1, 1).x *  uint2(8, 8).x)));
DispatchSizeY =  uint(((710) + (( uint2(1, 1).y *  uint2(8, 8).y)) - 1) / (( uint2(1, 1).y *  uint2(8, 8).y)));
}
#line 369
pass
{
