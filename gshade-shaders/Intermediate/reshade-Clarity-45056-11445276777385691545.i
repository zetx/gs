...
#line 83
texture ClarityTex < pooled = true; > { Width = 1500 * 0.5; Height = 1004 * 0.5; Format = R8; };
texture ClarityTex2 { Width = 1500 * 0.5; Height = 1004 * 0.5; Format = R8; };
texture ClarityTex3 < pooled = true; > { Width = 1500 * 0.25; Height = 1004 * 0.25; Format = R8; };
#line 87
...
for(int i = 1; i < 4; ++i)
{
color += tex2Dlod(ClaritySampler3, float4(texcoord + float2(0.0, offset[i] *  float2((1.0 / 1500), (1.0 / 1004)).y) * ClarityOffset, 0.0, 0.0)).r * weight[i];
color += tex2Dlod(ClaritySampler3, float4(texcoord - float2(0.0, offset[i] *  float2((1.0 / 1500), (1.0 / 1004)).y) * ClarityOffset, 0.0, 0.0)).r * weight[i];
}
}
#line 111
...
for(int i = 1; i < 4; ++i)
{
color += tex2Dlod(ReShade::BackBuffer, float4(texcoord + float2(offset[i] *  float2((1.0 / 1500), (1.0 / 1004)).x, 0.0) * ClarityOffset, 0.0, 0.0)).rgb * weight[i];
color += tex2Dlod(ReShade::BackBuffer, float4(texcoord - float2(offset[i] *  float2((1.0 / 1500), (1.0 / 1004)).x, 0.0) * ClarityOffset, 0.0, 0.0)).rgb * weight[i];
}
}
#line 293
...
for(int i = 1; i < 4; ++i)
{
color += tex2Dlod(ClaritySampler, float4(texcoord + float2(0.0, offset[i] *  float2((1.0 / 1500), (1.0 / 1004)).y) * ClarityOffset, 0.0, 0.0)).r* weight[i];
color += tex2Dlod(ClaritySampler, float4(texcoord - float2(0.0, offset[i] *  float2((1.0 / 1500), (1.0 / 1004)).y) * ClarityOffset, 0.0, 0.0)).r* weight[i];
}
}
#line 375
...
for(int i = 1; i < 4; ++i)
{
color += tex2Dlod(ClaritySampler2, float4(texcoord + float2(offset[i] *  float2((1.0 / 1500), (1.0 / 1004)).x, 0.0) * ClarityOffset, 0.0, 0.0)).r* weight[i];
color += tex2Dlod(ClaritySampler2, float4(texcoord - float2(offset[i] *  float2((1.0 / 1500), (1.0 / 1004)).x, 0.0) * ClarityOffset, 0.0, 0.0)).r* weight[i];
}
}
#line 457
