...
#line 137
texture alInTex  { Width = 1799 / 16; Height = 995 / 16; Format = RGBA32F; };
texture alOutTex { Width = 1799 / 16; Height = 995 / 16; Format = RGBA32F; };
...
#line 252
[flatten]
if ((texcoord.x + sampleOffsets[1] *  float2(1.0f / (1799 / 16.0f), 1.0f / (995 / 16.0f)).x) < 1.05)
hgb += tex2D(alInColor, texcoord + float2(sampleOffsets[1] *  float2(1.0f / (1799 / 16.0f), 1.0f / (995 / 16.0f)).x, 0.0)) * sampleWeights[1] * stepMult;
[flatten]
if ((texcoord.x - sampleOffsets[1] *  float2(1.0f / (1799 / 16.0f), 1.0f / (995 / 16.0f)).x) > -0.05)
hgb += tex2D(alInColor, texcoord - float2(sampleOffsets[1] *  float2(1.0f / (1799 / 16.0f), 1.0f / (995 / 16.0f)).x, 0.0)) * sampleWeights[1] * stepMult;
#line 259
[flatten]
if ((texcoord.x + sampleOffsets[2] *  float2(1.0f / (1799 / 16.0f), 1.0f / (995 / 16.0f)).x) < 1.05)
hgb += tex2D(alInColor, texcoord + float2(sampleOffsets[2] *  float2(1.0f / (1799 / 16.0f), 1.0f / (995 / 16.0f)).x, 0.0)) * sampleWeights[2] * stepMult;
[flatten]
if ((texcoord.x - sampleOffsets[2] *  float2(1.0f / (1799 / 16.0f), 1.0f / (995 / 16.0f)).x) > -0.05)
hgb += tex2D(alInColor, texcoord - float2(sampleOffsets[2] *  float2(1.0f / (1799 / 16.0f), 1.0f / (995 / 16.0f)).x, 0.0)) * sampleWeights[2] * stepMult;
#line 266
[flatten]
if ((texcoord.x + sampleOffsets[3] *  float2(1.0f / (1799 / 16.0f), 1.0f / (995 / 16.0f)).x) < 1.05)
hgb += tex2D(alInColor, texcoord + float2(sampleOffsets[3] *  float2(1.0f / (1799 / 16.0f), 1.0f / (995 / 16.0f)).x, 0.0)) * sampleWeights[3] * stepMult;
[flatten]
if ((texcoord.x - sampleOffsets[3] *  float2(1.0f / (1799 / 16.0f), 1.0f / (995 / 16.0f)).x) > -0.05)
hgb += tex2D(alInColor, texcoord - float2(sampleOffsets[3] *  float2(1.0f / (1799 / 16.0f), 1.0f / (995 / 16.0f)).x, 0.0)) * sampleWeights[3] * stepMult;
#line 273
[flatten]
if ((texcoord.x + sampleOffsets[4] *  float2(1.0f / (1799 / 16.0f), 1.0f / (995 / 16.0f)).x) < 1.05)
hgb += tex2D(alInColor, texcoord + float2(sampleOffsets[4] *  float2(1.0f / (1799 / 16.0f), 1.0f / (995 / 16.0f)).x, 0.0)) * sampleWeights[4] * stepMult;
[flatten]
if ((texcoord.x - sampleOffsets[4] *  float2(1.0f / (1799 / 16.0f), 1.0f / (995 / 16.0f)).x) > -0.05)
hgb += tex2D(alInColor, texcoord - float2(sampleOffsets[4] *  float2(1.0f / (1799 / 16.0f), 1.0f / (995 / 16.0f)).x, 0.0)) * sampleWeights[4] * stepMult;
}
...
#line 289
[flatten]
if ((texcoord.y + sampleOffsets[1] *  float2(1.0f / (1799 / 16.0f), 1.0f / (995 / 16.0f)).y) < 1.05)
vgb += tex2D(alOutColor, texcoord + float2(0.0, sampleOffsets[1] *  float2(1.0f / (1799 / 16.0f), 1.0f / (995 / 16.0f)).y)) * sampleWeights[1] * stepMult;
[flatten]
if ((texcoord.y - sampleOffsets[1] *  float2(1.0f / (1799 / 16.0f), 1.0f / (995 / 16.0f)).y) > -0.05)
vgb += tex2D(alOutColor, texcoord - float2(0.0, sampleOffsets[1] *  float2(1.0f / (1799 / 16.0f), 1.0f / (995 / 16.0f)).y)) * sampleWeights[1] * stepMult;
#line 296
[flatten]
if ((texcoord.y + sampleOffsets[2] *  float2(1.0f / (1799 / 16.0f), 1.0f / (995 / 16.0f)).y) < 1.05)
vgb += tex2D(alOutColor, texcoord + float2(0.0, sampleOffsets[2] *  float2(1.0f / (1799 / 16.0f), 1.0f / (995 / 16.0f)).y)) * sampleWeights[2] * stepMult;
[flatten]
if ((texcoord.y - sampleOffsets[2] *  float2(1.0f / (1799 / 16.0f), 1.0f / (995 / 16.0f)).y) > -0.05)
vgb += tex2D(alOutColor, texcoord - float2(0.0, sampleOffsets[2] *  float2(1.0f / (1799 / 16.0f), 1.0f / (995 / 16.0f)).y)) * sampleWeights[2] * stepMult;
#line 303
[flatten]
if ((texcoord.y + sampleOffsets[3] *  float2(1.0f / (1799 / 16.0f), 1.0f / (995 / 16.0f)).y) < 1.05)
vgb += tex2D(alOutColor, texcoord + float2(0.0, sampleOffsets[3] *  float2(1.0f / (1799 / 16.0f), 1.0f / (995 / 16.0f)).y)) * sampleWeights[3] * stepMult;
[flatten]
if ((texcoord.y - sampleOffsets[3] *  float2(1.0f / (1799 / 16.0f), 1.0f / (995 / 16.0f)).y) > -0.05)
vgb += tex2D(alOutColor, texcoord - float2(0.0, sampleOffsets[3] *  float2(1.0f / (1799 / 16.0f), 1.0f / (995 / 16.0f)).y)) * sampleWeights[3] * stepMult;
#line 310
[flatten]
if ((texcoord.y + sampleOffsets[4] *  float2(1.0f / (1799 / 16.0f), 1.0f / (995 / 16.0f)).y) < 1.05)
vgb += tex2D(alOutColor, texcoord + float2(0.0, sampleOffsets[4] *  float2(1.0f / (1799 / 16.0f), 1.0f / (995 / 16.0f)).y)) * sampleWeights[4] * stepMult;
[flatten]
if ((texcoord.y - sampleOffsets[4] *  float2(1.0f / (1799 / 16.0f), 1.0f / (995 / 16.0f)).y) > -0.05)
vgb += tex2D(alOutColor, texcoord - float2(0.0, sampleOffsets[4] *  float2(1.0f / (1799 / 16.0f), 1.0f / (995 / 16.0f)).y)) * sampleWeights[4] * stepMult;
}
#line 318
...
dither = lerp(2.0 * dither, -2.0 * dither, frac(dot(texcoord,  float2(1799, 995) * float2(1.0 / 16.0, 10.0 / 36.0)) + 0.25));
}
#line 458
if (all(base.xyz == 1.0))
{
