...
texture PlaceHolder {Width = 1280; Height = 720; Format = RGB10A2;};
texture Convolution {Width = 1280; Height = 720; Format = RGB10A2;};
#line 57
...
sampleCoord.x += i;
sampleCoord = clamp(sampleCoord, 0, float2(1280, 720));
...
int2 sampleCoord = sampleOffset + groupCoord;
sampleCoord.x += i;
sampleCoord = clamp(sampleCoord, 0, float2(720, 1280));
...
DispatchSizeX =  (int(1280 +  32 *  4 - 1) / int( 32 *  4));
DispatchSizeY =  (int(720 +  2 - 1) / int( 2));
}
pass
{
ComputeShader = VerticalPassCS< 32,  2>;
DispatchSizeX =  (int(720 +  32 *  4 - 1) / int( 32 *  4));
DispatchSizeY =  (int(1280 +  2 - 1) / int( 2));
}
pass
{
