...
> = false;
#line 47
texture		TexBloomH { Width = 1799* 	0.25			; Height = 995* 	0.25			; Format = RGBA8;};
texture		TexBloomV { Width = 1799* 	0.25			; Height = 995* 	0.25			; Format = RGBA8;};
...
for(int i=1; i < 	12			; i++)
{
ret += max(tex2D(ReShade::BackBuffer, coords + float2(i *   	float2((1.0 / 1799), (1.0 / 995)).x * BLOOM_RADIUS, 0.0)) - BLOOM_THRESHOLD, 0.0);
ret += max(tex2D(ReShade::BackBuffer, coords - float2(i *   	float2((1.0 / 1799), (1.0 / 995)).x * BLOOM_RADIUS, 0.0)) - BLOOM_THRESHOLD, 0.0);
}
#line 70
...
for(int i=1; i < 	12			; i++)
{
ret += tex2D(SamplerBloomV, coords + float2(i *   	float2((1.0 / 1799), (1.0 / 995)).x * BLOOM_RADIUS, 0.0));
ret += tex2D(SamplerBloomV, coords - float2(i *   	float2((1.0 / 1799), (1.0 / 995)).x * BLOOM_RADIUS, 0.0));
}
#line 82
return ret / 	((	12			 * 2) - 1);
...
for(int i=1; i < 	12			; i++)
{
ret += tex2D(SamplerBloomH, coords + float2(0.0, i *   	float2((1.0 / 1799), (1.0 / 995)).y * BLOOM_RADIUS));
ret += tex2D(SamplerBloomH, coords - float2(0.0, i *   	float2((1.0 / 1799), (1.0 / 995)).y * BLOOM_RADIUS));
}
#line 94
return ret / 	((	12			 * 2) - 1);
