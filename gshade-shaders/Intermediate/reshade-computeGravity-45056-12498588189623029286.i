...
#line 151
texture texGravitySeedMap{ Width =  1798/ 1; Height =  768; Format = R16F; };
texture texGravitySeedMapCopy{ Width =  1798/ 1; Height =  768; Format = R16F; };
...
texture texGravityMain{ Width =  1798/ 1; Height =  768; Format = RGBA8; };
texture texGravityDepth{ Width =  1798/ 1; Height =  768; Format = R32F; };
#line 162
...
#line 286
finalist[y] = y;
depthlist[y] = depthlistU[y] = ReShade::GetLinearizedDepth(float2((id.x* 1 + 0.5) / 1798, (y *  997/ 768 + 0.5) / 997));
}
...
#line 295
float4 rgbval =  tex2Dfetch(ReShade::BackBuffer, int2(id.x* 1, yi* 997/ 768), 0); 
float scenedepth = depthlist[yi]; 
float strength = tex2Dfetch(SamplerGravitySeedMap, int2(id.x, yi), 0).r; 
strength = strength * GravityIntensity * inFocus(rgbval, scenedepth, float2((id.x* 1 + 0.5) / 1798, (yi *  997/ 768 + 0.5) / 997)); 
...
{
if (y != finalist[y]) {
float4 storeVal = tex2Dfetch(ReShade::BackBuffer, int2(id.x, finalist[y] *  997/ 768), 0); 
...
pass GravityMain { ComputeShader = gravityMain<1, 1>; DispatchSizeX =  1798/ 1; DispatchSizeY = 1; }
