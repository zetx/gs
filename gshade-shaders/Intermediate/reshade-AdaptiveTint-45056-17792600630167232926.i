...
#line 12
namespace Stats {
texture2D shared_texStats { Width = 5360; Height = 1440; Format = RGBA8; MipLevels =   7.0; };
...
float fac = 0.0;
#line 422
float2 coord_pix = float2(5360, 1440) * texcoord;
...
float valN = dot(tex2D(s, texcoord + float2(0.0, - float2((1.0 / 5360), (1.0 / 1440)).y)).rgb,  float3(0.2126, 0.7151, 0.0721));
float valNE = dot(tex2D(s, texcoord + float2( float2((1.0 / 5360), (1.0 / 1440)).x, - float2((1.0 / 5360), (1.0 / 1440)).y)).rgb,  float3(0.2126, 0.7151, 0.0721));
float valE = dot(tex2D(s, texcoord + float2( float2((1.0 / 5360), (1.0 / 1440)).x, 0.0)).rgb,  float3(0.2126, 0.7151, 0.0721));
float valSE = dot(tex2D(s, texcoord + float2( float2((1.0 / 5360), (1.0 / 1440)).x,  float2((1.0 / 5360), (1.0 / 1440)).y)).rgb,  float3(0.2126, 0.7151, 0.0721));
float valS = dot(tex2D(s, texcoord + float2(0.0,  float2((1.0 / 5360), (1.0 / 1440)).y)).rgb,  float3(0.2126, 0.7151, 0.0721));
float valSW = dot(tex2D(s, texcoord + float2(- float2((1.0 / 5360), (1.0 / 1440)).x,  float2((1.0 / 5360), (1.0 / 1440)).y)).rgb,  float3(0.2126, 0.7151, 0.0721));
float valW = dot(tex2D(s, texcoord + float2(- float2((1.0 / 5360), (1.0 / 1440)).x, 0.0)).rgb,  float3(0.2126, 0.7151, 0.0721));
float valNW = dot(tex2D(s, texcoord + float2(- float2((1.0 / 5360), (1.0 / 1440)).x, - float2((1.0 / 5360), (1.0 / 1440)).y)).rgb,  float3(0.2126, 0.7151, 0.0721));
#line 486
...
float depthN =  ReShade::GetLinearizedDepth(texcoord + float2(0.0, - float2((1.0 / 5360), (1.0 / 1440)).y));
float depthNE = ReShade::GetLinearizedDepth(texcoord + float2( float2((1.0 / 5360), (1.0 / 1440)).x, - float2((1.0 / 5360), (1.0 / 1440)).y));
float depthE =  ReShade::GetLinearizedDepth(texcoord + float2( float2((1.0 / 5360), (1.0 / 1440)).x, 0.0));
float depthSE = ReShade::GetLinearizedDepth(texcoord + float2( float2((1.0 / 5360), (1.0 / 1440)).x,  float2((1.0 / 5360), (1.0 / 1440)).y));
float depthS =  ReShade::GetLinearizedDepth(texcoord + float2(0.0,  float2((1.0 / 5360), (1.0 / 1440)).y));
float depthSW = ReShade::GetLinearizedDepth(texcoord + float2(- float2((1.0 / 5360), (1.0 / 1440)).x,  float2((1.0 / 5360), (1.0 / 1440)).y));
float depthW =  ReShade::GetLinearizedDepth(texcoord + float2(- float2((1.0 / 5360), (1.0 / 1440)).x, 0.0));
float depthNW = ReShade::GetLinearizedDepth(texcoord + float2(- float2((1.0 / 5360), (1.0 / 1440)).x, - float2((1.0 / 5360), (1.0 / 1440)).y));
...
float fac = 0.0;
#line 26
float2 screencoord = float2(5360, 1440) * texcoord;
float2 overlay_size = (float2)tex2Dsize(overlay, 0);
offset.x = clamp(offset.x, 0, 5360 - overlay_size.x);
...
> = 1.0;
#line 114
 uniform int2 AdaptiveTintDebugPosition < ui_type = "slider"; ui_category = "AdaptiveTintDebug"; ui_label = "Position"; ui_min = 0; ui_max = 5360; ui_step = 1; > = int2(0, 0); uniform float AdaptiveTintDebugOpacity < ui_type = "slider"; ui_category = "AdaptiveTintDebug"; ui_label = "Opacity"; ui_min = 0.0; ui_max = 1.0; ui_step = 0.01; > = 1.0; texture2D  texAdaptiveTintDebug { Width = 5360/4; Height = 1440/4; Format = RGBA8; }; sampler2D  sAdaptiveTintDebug { Texture =  texAdaptiveTintDebug; }; float3  AdaptiveTintDebugoverlay(float4 vpos : SV_Position, float2 texcoord : TexCoord) : SV_Target { float3 backbuffer = tex2D(ReShade::BackBuffer, texcoord).rgb; return Canvas::OverlaySampler(backbuffer,  sAdaptiveTintDebug, texcoord, AdaptiveTintDebugPosition, AdaptiveTintDebugOpacity); }
#line 119
texture2D texAlphaCheckerboard < source = "alpha-checkerboard.png"; > { Width = 5360; Height = 1440; Format = RGBA8; };
...
#line 205
  ...
  AdaptiveTintDebug = Canvas::DrawScale(AdaptiveTintDebug, 	float3(0.0, 0.0, 0.0), 	float3(1.0, 1.0, 1.0), int2(10, 0), int2(5360/4-10, 10), avgLuma,  float3(1.0, 0.0, 1.0), float2(texcoord.x,  1.0 - texcoord.y),  sAdaptiveTintDebug, 0.002);;
