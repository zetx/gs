...
#line 204
float2 uv = (tex * float2(3440, 1440) - pos) / size;
uv.y      = saturate(uv.y);
uv.x     *= ratio * 2.0;
#line 208
...
{
int2 pixcoord = floor(( float2(3440, 1440) * texcoord) / cell_size) * cell_size;
color =  tex2D(ReShade::BackBuffer, ((pixcoord) + 0.5) *  float2((1.0 / 3440), (1.0 / 1440)));
#line 36
if(avg_amount > 0.1)
{
...
for( int x = 0 ; x < 4 ; ++x )
for( int y = 0 ; y < 4 ; ++y )
avg_color +=  tex2D(ReShade::BackBuffer, ((float2(pixcoord.x + (x * step), pixcoord.y + (y * step))) + 0.5) *  float2((1.0 / 3440), (1.0 / 1440)));
#line 45
avg_color *= 0.0625;
...
if(stats)
{
DrawText_Digit(float2( float2(3440, 1440).x - 256, 128), 64, 1, texcoord, -1, ( float2(3440, 1440).x / cell_size), color.x);
DrawText_Digit(float2( float2(3440, 1440).x - 256, 192), 64, 1, texcoord, -1, ( float2(3440, 1440).y / cell_size), color.x);
}
}
#line 56
technique Pixelate
