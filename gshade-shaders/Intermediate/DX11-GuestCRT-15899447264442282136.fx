...
#line 361
texture Texture1GCRT{Width = 3440; Height = 1440; Format = RGBA16F;};
sampler Sampler1GCRT{Texture = Texture1GCRT; MinFilter = Linear; MagFilter = Linear;};
#line 364
texture Texture2GCRT{Width = 3440; Height = 1440; Format = RGBA16F;};
...
coord = min(coord, 1.0 - coord) * float2(1.0,  float4( float2(3440, 1440), 1.0 /  float2(3440, 1440)).y /  float4( float2(3440, 1440), 1.0 /  float2(3440, 1440)).x);
...
if(IOS > 0.0)
{
const float2 ofactor =  float4( float2(3440, 1440), 1.0 /  float2(3440, 1440)).xy /  float2( 320,  240).xy;
