...
#line 132
texture2D	TexNormalDepth {Width = 1309 * 		0.5	; Height = 762 * 		0.5	; Format = RGBA16; MipLevels = 		5	;};
...
vec.y -= vec.y * (1.0 - vec.z) * sin(radians(FOV * (  	float2((1.0 / 1309), (1.0 / 762)).y /   	float2((1.0 / 1309), (1.0 / 762)).x)));
return vec;
}
#line 166
float4 GetNormalDepth(float2 coords)
{
const float2 offsety = float2(0.0,   	float2((1.0 / 1309), (1.0 / 762)).y);
const float2 offsetx = float2(  	float2((1.0 / 1309), (1.0 / 762)).x, 0.0);
#line 171
...
if (discradius <   	float2((1.0 / 1309), (1.0 / 762)).x / 		0.5	)
return origcolor;
const float2 calcv = v * discradius *   	float2((1.0 / 1309), (1.0 / 762)) / 		0.5	;
#line 303
for(int i=1; i <= 			4	; i++)
{
#line 307
