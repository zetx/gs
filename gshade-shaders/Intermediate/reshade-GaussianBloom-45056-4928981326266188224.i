...
#line 71
texture GaussianBloomTex { Width = 1798* 0.5; Height = 997* 0.5; Format = RGBA8; };
texture GaussianBloomTex2 { Width = 1798* 0.5; Height = 997* 0.5; Format = RGBA8; };
#line 74
...
for(int i = 1; i < 4; ++i)
{
blur += tex2Dlod(GaussianBloomSampler, float4(texcoord + float2(0.0, offset[i] *  float2((1.0 / 1798), (1.0 / 997)).y), 0.0, 0.0)).rgb * weight[i];
blur += tex2Dlod(GaussianBloomSampler, float4(texcoord - float2(0.0, offset[i] *  float2((1.0 / 1798), (1.0 / 997)).y), 0.0, 0.0)).rgb * weight[i];
}
}
#line 139
...
for(int i = 1; i < 6; ++i)
{
blur += tex2Dlod(GaussianBloomSampler2, float4(texcoord + float2( offset[i] *  float2((1.0 / 1798), (1.0 / 997)).x,0.0), 0.0, 0.0)).rgb * weight[i];
blur += tex2Dlod(GaussianBloomSampler2, float4(texcoord - float2( offset[i] *  float2((1.0 / 1798), (1.0 / 997)).x,0.0), 0.0, 0.0)).rgb * weight[i];
}
}
#line 222
