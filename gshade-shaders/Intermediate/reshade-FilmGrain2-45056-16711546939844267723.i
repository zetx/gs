...
{
float rotX = ((tc.x * 2.0 - 1.0) *  (1500 * (1.0 / 1004)) * cos(angle)) - ((tc.y * 2.0 - 1.0) * sin(angle));
float rotY = ((tc.y * 2.0 - 1.0) * cos(angle)) + ((tc.x * 2.0 - 1.0) *  (1500 * (1.0 / 1004)) * sin(angle));
rotX = ((rotX /  (1500 * (1.0 / 1004))) * 0.5 + 0.5);
rotY = rotY * 0.5 + 0.5;
#line 113
...
float3 noise = pnoise3D(float3(rotCoordsR *  float2(1500, 1004) / grainsize, 0.0)).xxx;
#line 122
if (coloramount > 0)
{
float2 rotCoordsG = coordRot(texCoord, timer + rotOffset.y);
float2 rotCoordsB = coordRot(texCoord, timer + rotOffset.z);
noise.g = lerp(noise.r, pnoise3D(float3(rotCoordsG *  float2(1500, 1004) / grainsize, 1.0)), coloramount);
noise.b = lerp(noise.r, pnoise3D(float3(rotCoordsB *  float2(1500, 1004) / grainsize, 2.0)), coloramount);
}
#line 130
