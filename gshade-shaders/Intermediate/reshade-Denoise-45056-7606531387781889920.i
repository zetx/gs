...
for (int i = - 3	; i <=  3	; i++) {
for (int j = - 3	; j <=  3	; j++) {
texIJ = tex2Dlod(ReShade::BackBuffer, float4(texcoord +  float2((1.0 / 1798), (1.0 / 997)) * float2(i, j), 0.0, 0.0)).rgb;
...
for (int n = - 2	; n <=  2	; n++) {
for (int m = - 2	; m <=  2	; m++) {
texIJb = tex2Dlod(ReShade::BackBuffer, float4(texcoord +  float2((1.0 / 1798), (1.0 / 997)) * float2(i + n, j + m), 0.0, 0.0)).rgb;
texIJc = tex2Dlod(ReShade::BackBuffer, float4(texcoord +  float2((1.0 / 1798), (1.0 / 997)) * float2(    n,     m), 0.0, 0.0)).rgb;
weight = dot(texIJb - texIJc, texIJb - texIJc) + weight;
}
}
texIJc = tex2Dlod(ReShade::BackBuffer, float4(texcoord +  float2((1.0 / 1798), (1.0 / 997)) * float2(i, j), 0.0, 0.0)).rgb;
#line 220
weight *= invBlockArea;
weight = exp(-(weight * rcp(NoiseLevel) + (i * i + j * j) * rcp(GaussianSigma)));
#line 223
