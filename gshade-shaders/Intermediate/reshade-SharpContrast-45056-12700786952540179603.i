...
texture Luma<pooled = true;>{Width = 1799; Height = 995; Format = R8;};
texture Temp<pooled = true;>{Width = 1799; Height = 995; Format = R8;};
#line 101
...
{
float4 r = tex2DgatherR(sBackBuffer, (float2(id.xy * 2) + 0.5)/float2(1799, 995));
float4 g = tex2DgatherG(sBackBuffer, (float2(id.xy * 2) + 0.5)/float2(1799, 995));
float4 b = tex2DgatherB(sBackBuffer, (float2(id.xy * 2) + 0.5)/float2(1799, 995));
#line 176
float4 luma;
...
(sampleCoord > float2(1799, 995)) ? float2(1799, 995) - (sampleCoord - float2(1799, 995)) : sampleCoord;
#line 207
sampleCoord /= float2(1799, 995);
#line 209
...
DispatchSizeY =  uint(((995) + (16) - 1) / (16));
}
pass
{
...
DispatchSizeY =  uint(((995) + (( uint2(1, 1).y *  uint2(8, 8).y)) - 1) / (( uint2(1, 1).y *  uint2(8, 8).y)));
}
#line 369
pass
{
