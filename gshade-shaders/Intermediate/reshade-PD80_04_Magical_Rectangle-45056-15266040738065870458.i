...
> = 1.0;
#line 217
texture texMagicRectangle { Width = 1799; Height = 998; Format = RGBA16F; };
#line 220
...
uv.y         = ( id == 1 ) ? 2.0 : 0.0;
uv.xy        -= center.xy;
uv.y         /=  float( 1799 * (1.0 / 998) );
float dim    = ceil( sqrt( 1799 * 1799 + 998 * 998 )); 
float maxlen = min( 1799, 998 );
dim          = dim / maxlen; 
uv.xy        /= dim;
