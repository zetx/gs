...
#line 286
finalist[y] = y;
depthlist[y] = depthlistU[y] = ReShade::GetLinearizedDepth(float2((id.x* 1 + 0.5) / 1799, (y *  998/ 768 + 0.5) / 998));
}
...
#line 295
float4 rgbval =  tex2Dfetch(ReShade::BackBuffer, int2(id.x* 1, yi* 998/ 768), 0); 
float scenedepth = depthlist[yi]; 
float strength = tex2Dfetch(SamplerGravitySeedMap, int2(id.x, yi), 0).r; 
strength = strength * GravityIntensity * inFocus(rgbval, scenedepth, float2((id.x* 1 + 0.5) / 1799, (yi *  998/ 768 + 0.5) / 998)); 
...
{
if (y != finalist[y]) {
float4 storeVal = tex2Dfetch(ReShade::BackBuffer, int2(id.x, finalist[y] *  998/ 768), 0); 
