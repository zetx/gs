...
#line 171
texture2D MXAO_ColorTex 	{ Width = 1024;   Height = 768;   Format = RGBA8; MipLevels = 3+		2	;};
texture2D MXAO_DepthTex 	{ Width = 1024;   Height = 768;   Format = R16F;  MipLevels = 3+		0	;};
texture2D MXAO_NormalTex	{ Width = 1024;   Height = 768;   Format = RGBA8; MipLevels = 3+		2	;};
texture2D MXAO_CullingTex	{ Width = 1024/8; Height = 768/8; Format = R8; };
#line 176
...
{
const float2 sampleCoord = MXAO.texcoord.xy + blurOffsets[iStep] *  float2((1.0 / 1024), (1.0 / 768)) * radius / inputscale;
#line 303
...
{
const float2 searchRadiusScaled = 0.018 / P.z * float2(1.0, (1024 * (1.0 / 768)));
...
{
const float4 cOffsets = float4( float2((1.0 / 1024), (1.0 / 768)),- float2((1.0 / 1024), (1.0 / 768))) * 8;
...
{
const float3 offs = float3( float2((1.0 / 1024), (1.0 / 768)),0);
#line 405
...
{
sampleUV = MXAO.scaledcoord.xy + Dir.xy * float2(1.0,  (1024 * (1.0 / 768))) * (iSample + randStep);
...
|| 0.25 * 0.5 * MXAO_SAMPLE_RADIUS / (tex2D(sMXAO_DepthTex,MXAO.scaledcoord.xy).x + 2.0) * 768 < 1.0
