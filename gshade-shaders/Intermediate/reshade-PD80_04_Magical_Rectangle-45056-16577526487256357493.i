...
> = 1.0;
#line 217
texture texMagicRectangle { Width = 1024; Height = 768; Format = RGBA16F; };
#line 220
...
uv.y         = ( id == 1 ) ? 2.0 : 0.0;
uv.xy        -= center.xy;
uv.y         /=  float( 1024 * (1.0 / 768) );
float dim    = ceil( sqrt( 1024 * 1024 + 768 * 768 )); 
float maxlen = min( 1024, 768 );
dim          = dim / maxlen; 
uv.xy        /= dim;
