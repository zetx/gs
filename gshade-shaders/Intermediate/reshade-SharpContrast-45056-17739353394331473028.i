...
texture Luma<pooled = true;>{Width = 1281; Height = 721; Format = R8;};
texture Temp<pooled = true;>{Width = 1281; Height = 721; Format = R8;};
#line 101
...
{
float4 r = tex2DgatherR(sBackBuffer, (float2(id.xy * 2) + 0.5)/float2(1281, 721));
float4 g = tex2DgatherG(sBackBuffer, (float2(id.xy * 2) + 0.5)/float2(1281, 721));
float4 b = tex2DgatherB(sBackBuffer, (float2(id.xy * 2) + 0.5)/float2(1281, 721));
#line 176
float4 luma;
...
(sampleCoord > float2(1281, 721)) ? float2(1281, 721) - (sampleCoord - float2(1281, 721)) : sampleCoord;
#line 207
sampleCoord /= float2(1281, 721);
#line 209
...
DispatchSizeX =  uint(((1281) + (16) - 1) / (16));
DispatchSizeY =  uint(((721) + (16) - 1) / (16));
}
pass
{
ComputeShader = KuwaharaSharpCS< uint2(8, 8).x,  uint2(8, 8).y>;
DispatchSizeX =  uint(((1281) + (( uint2(1, 1).x *  uint2(8, 8).x)) - 1) / (( uint2(1, 1).x *  uint2(8, 8).x)));
DispatchSizeY =  uint(((721) + (( uint2(1, 1).y *  uint2(8, 8).y)) - 1) / (( uint2(1, 1).y *  uint2(8, 8).y)));
}
#line 369
pass
{
