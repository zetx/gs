...
#line 151
texture2D InterleavedAOTex  { Width = 792 / 2;   Height = 710 / 2;   Format = R8; MipLevels = 1;};
texture2D InterleavedAOTex2 { Width = 792 / 2;   Height = 710 / 2;   Format = R8; MipLevels = 1;};
texture2D InterleavedAOTex3 { Width = 792 / 2;   Height = 710 / 2;   Format = R8; MipLevels = 1;};
texture2D InterleavedAOTex4 { Width = 792 / 2;   Height = 710 / 2;   Format = R8; MipLevels = 1;};
#line 156
texture2D AOTex	{ Width = 792;   Height = 710;   Format = R8; MipLevels = 1;};
texture2D AOTex2	{ Width = 792;   Height = 710;   Format = R8; MipLevels = 1;};
texture2D NormalTex	{ Width = 792;   Height = 710;   Format = RGBA8; MipLevels = 1;};
#line 160
...
#line 190
const float2 offx = float2( float2((1.0 / 792), (1.0 / 710)).x, 0);
const float2 offy = float2(0,  float2((1.0 / 792), (1.0 / 710)).y);
#line 193
...
#line 237
float2 off = float2( float2((1.0 / 792), (1.0 / 710)).x, 0);
#line 239
float4 my_normal;
#line 241
if (bilateral)
...
if (Bilateral == 2)
{
off = float2(0,  float2((1.0 / 792), (1.0 / 710)).y);
}
#line 251
float weights[32];
weights[0] = 1;
...
range *= 3.0 * clamp(BlurQuality, 0.0, 1.0);
#line 340
float2 off = float2(0,  float2((1.0 / 792), (1.0 / 710)).y);
#line 342
float4 my_normal;
#line 344
if (bilateral)
...
if (Bilateral == 2)
{
off = float2( float2((1.0 / 792), (1.0 / 710)).x, 0);
}
#line 354
float weights[32];
weights[0] = 1;
...
{
const float2 ray_in_pixels = ray /  float2((1.0 / 792), (1.0 / 710));
...
{
uint2 texcoord_px = texcoord /  float2((1.0 / 792), (1.0 / 710));
#line 456
...
ray.x = sin(angle);
ray.y = cos(angle);
ray *=  float2((1.0 / 792), (1.0 / 710)) * sample_dist;
...
{
occlusion_1 = GetOcclusion(texcoord + float2(-0.25,  0.25) *  float2((1.0 / 792), (1.0 / 710)),  0.00);
occlusion_2 = GetOcclusion(texcoord + float2( 0.25,  0.25) *  float2((1.0 / 792), (1.0 / 710)),  0.25);
occlusion_3 = GetOcclusion(texcoord + float2(-0.25, -0.25) *  float2((1.0 / 792), (1.0 / 710)),  0.50);
occlusion_4 = GetOcclusion(texcoord + float2( 0.25, -0.25) *  float2((1.0 / 792), (1.0 / 710)),  0.75);
}
#line 560
technique MC_DAO
{
