...
{
return tex2D(samp, uv - fmod(uv, float2(pixelSize,pixelSize) /  float2(792, 710)));
}
#line 72
float rand(float n)
{
...
{
const float dx = sampleSize * (1.0 / 792);
const float dy = sampleSize * (1.0 / 710);
return (
...
{
const float2 pixel =  float2((1.0 / 792), (1.0 / 710));
const float3 field = rgb2hsv(edge(samp, uv, edgeSize));
const float2 distort = pixel * sin((field.rb) *  3.1415926535897932 * 2.0);
const float shiftx = noise(float2(quantize(uv.y + 31.5, 710 /  16.0) * Timer*0.001, frac(Timer*0.001) * 300.0));
const float shifty = noise(float2(quantize(uv.x + 11.5, 792 /  16.0) * Timer*0.001, frac(Timer*0.001) * 100.0));
...
float Amount;
#line 130
const float2 texcoord = fragCoord *  float2(792, 710); 
const float2 uv = texcoord.xy /  float2(792, 710);
if (bUseUV) {
Amount = uv.x; 
}
