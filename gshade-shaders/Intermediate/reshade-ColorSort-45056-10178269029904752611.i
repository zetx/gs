...
{
#line 118
texture texHalfRes{ Width = 1309; Height = 	640 ; Format = RGBA16F; };
texture texColorSort{ Width = 1309; Height = 	640 ; Format = RGBA16F; };
...
float scenedepth = ReShade::GetLinearizedDepth(float2((id.x+0.5) / float(1309), (i+0.5) / float(	640 )));
is_focus = inFocus(colortable[i + tid.x*	640 ], scenedepth, float2((id.x + 0.5) / float(1309), (i + 0.5) / float(	640 )));
#line 263
...
pass sortColor { ComputeShader = sort_color<2, 	16 >; DispatchSizeX = 1309 / 2; DispatchSizeY = 1; }
